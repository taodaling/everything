...
public class BZOJ4129 {
    public static void main(String[] args) throws FileNotFoundException {
        boolean local = System.getProperty("ONLINE_JUDGE") == null;
        IOUtil io;
        if (local) {
            io = new IOUtil(new FileInputStream("E:\\DATABASE\\TESTCASE\\BZOJ4129.in"), new FileOutputStream("E:\\DATABASE\\TESTCASE\\BZOJ4129.out"));
        } else {
    ...
    public static class Task implements Runnable {
        ...
        public void run() {
            ...
            ModifiableTreeMOAlgorithm<Integer> mo = new ModifiableTreeMOAlgorithm(n);
            for (int i = 1; i <= n; i++) {
                values[i] = io.readInt();
                mo.setNodeValue(i, values[i]);
            }
            ...
            for (int i = 0; i < m; i++) {
                ...
                if (t == 0) {
                    modificationList.add(new ModificationImpl(u, values[u], v));
                    values[u] = v;
                    version++;
                } else {
                    queryList.add(new QueryImpl(u, v, version));
                }
            }

            QueryImpl[] queries = queryList.toArray(new QueryImpl[0]);
            mo.buildFinish();
            mo.solve(queries, new IntervalImpl(), modificationList.toArray(new ModificationImpl[0]));
    ...
    public static class IntervalImpl implements ModifiableTreeMOAlgorithm.Interval<Integer> {
        int MAX = (int) 5e4;
        int[] cnt = new int[MAX + 1];
        int part = 223;
        int group = 225;
        int[] summary = new int[group + 1];
    ...
    public static class QueryImpl implements ModifiableTreeMOAlgorithm.OutsideQuery {
        final int u;
        final int v;
        final int version;
        int ans;

        public QueryImpl(int u, int v, int version) {
            this.u = u;
            this.v = v;
        ...
        @Override
        public void setAnswer(ModifiableTreeMOAlgorithm.Interval interval) {
            ans = ((IntervalImpl) interval).mex();
        }
    ...
    public static class ModificationImpl implements ModifiableTreeMOAlgorithm.Modification<Integer> {
        final int index;
        final Integer origin;
        final Integer replacement;

        public ModificationImpl(int index, Integer origin, Integer replacement) {
            this.index = index;
            this.origin = origin;
    ...
    public static class ModifiableTreeMOAlgorithm<T> {
