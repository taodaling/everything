 ...
 */
public class LongLongMessage3 {
    static BlockReader input;
    String textOne;
    String textTwo;

    public static void main(String[] args) throws Exception {
        // System.setIn(new FileInputStream("D:\\DataBase\\TESTCASE\\poj\\GoingHome.in"));

        input = new BlockReader(System.in);
        while (input.hasMore()) {
            LongLongMessage3 goingHome = new LongLongMessage3();
            goingHome.init();
    ...
    public int solve() {
        char[] data = new StringBuilder(textOne.length() + textTwo.length() + 1)
                .append(textOne).append(textTwo).append((char) ('z' + 1)).toString().toCharArray();

        SuffixArray array = SuffixArray.makeSuffixArray(data, 'a', 'z' + 2);
        ...
        for (int i = 0, bound = data.length; i < bound; i++) {
            ...
            SuffixArray.Suffix suffix = array.getSuffixByStartIndex(i);
            SuffixArray.Suffix pre = array.getSuffixByRank(suffix.rank - 1);

            for (int si = suffix.suffixStartIndex, pi = pre.suffixStartIndex, degreeBound = data.length - Math.max(si, pi);
                 lastMatchDegree < degreeBound && data[si + lastMatchDegree] == data[pi + lastMatchDegree]; lastMatchDegree++)
                ;
            match[suffix.rank] = lastMatchDegree;
            if (suffix.suffixStartIndex < sepPost && pre.suffixStartIndex >= sepPost ||
                    suffix.suffixStartIndex >= sepPost && pre.suffixStartIndex < sepPost) {
    ...
    public static class SuffixArray {
        ...
        private SuffixArray(Suffix[] originalSuffix, Suffix[] orderedSuffix, char[] data) {
            this.originalSuffix = originalSuffix;
            this.orderedSuffix = orderedSuffix;
            this.data = data;
        }
        ...
        public static SuffixArray makeSuffixArray(char[] s, int rangeFrom, int rangeTo) {
            ...
            return new SuffixArray(originalSuffix, suffixLoop.get(), s.clone());
        }

        public static void assignRank(Suffix[] seq, int[] firstKeys, int[] secondKeys, int[] rankOutput) {
            int cnt = 0;
            rankOutput[0] = 0;
        ...
        public static void sort(Suffix[] oldSeq, Suffix[] newSeq, int[] withRank, int range) {
