...
public class CR46D {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        long end = 0;
        if (local) {
            io = new IOUtil(new FileInputStream("E:\\DATABASE\\TESTCASE\\CR46D.in"), new FileOutputStream("E:\\DATABASE\\TESTCASE\\CR46D.out"));
        ...
        public void run() {
            ...
            for (int i = n - 1; i >= 0; i--) {
                for (int j = 0; j < i; j++) {
                    ...
                    dp[j] = (dp[j] + choose(cnt, v) * (var + 1)) % modulo;
                }
                var = (var + dp[i]) % modulo;
            }
        ...
        public int choose(int i, int j) {
            ...
            if (!visited[i][j]) {
                ...
                if (j == 0) {
                    ...
                    comp[i][j] = (choose(i - 1, j - 1) + choose(i - 1, j)) % modulo;
                }
            }
