...
public class POJ1811V2 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            if (debug.allowDebug) {
                debug.assertTrue(Mathematics.modmul(2, 3, 4) == 2);
                debug.assertTrue(Mathematics.modmul(3, 5, 2) == 1);
                debug.assertTrue(isPrime(6) == false);
                debug.assertTrue(isPrime(11) == true);
                debug.assertTrue(isPrime(121) == false);
                debug.assertTrue(minFactor(2) == 2);
                debug.assertTrue(minFactor(6) == 2);
                debug.assertTrue(minFactor(8) == 2);
            }

            long n = io.readLong();
            long m = minFactor(n);
            if (n == m) {
                io.cache.append("Prime\n");
            } else {
                io.cache.append(m).append('\n');
            }
        }

        public long minFactor(long n) {
            if (isPrime(n)) {
                return n;
            }
            long f;
            while ((f = findFactor(n)) == -1) ;
            return Math.min(minFactor(f), minFactor(n / f));
        }

        public long findFactor(long n) {
            long xi = (long) (random.nextDouble() * n);
            long c = (long) (random.nextDouble() * n);
            long xh = 0;
            int h = 0;
            int i = 0;
            while (i < n) {
                i++;
                xi = Mathematics.modmul(xi, xi, n);
                xi = (xi + c) % n;
                long g = Mathematics.gcd(n, Math.abs(xi - xh));
                if (g != 1 && g != n) {
                    return g;
                }
                if (i >= h) {
        ...
        public boolean isPrime(long n) {
            ...
            return isPrime(n, 3) && isPrime(n, 7);
        }

        public boolean isPrime(long n, long x) {
            long p = n - 1;
            while (true) {
                long v = Mathematics.longpow(x, p, n);
                if (v != 1 && v != n - 1) {
                    return false;
                }
                if (v != 1 || (p & 1) == 1) {
