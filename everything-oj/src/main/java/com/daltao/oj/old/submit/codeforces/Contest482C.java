...
public class Contest482C {
    ...
    public static void init() throws FileNotFoundException {
        if (System.getProperty("ONLINE_JUDGE") == null) {
            input = new BlockReader(new FileInputStream("E:\\DATABASE\\TESTCASE\\codeforces\\Contest482C.in"));
            output = System.out;
        } else {
    ...
    public static void solve() {
        ...
        long total = (long) n * (long)(n - 1);
        Node trace = nodes[u];
        while (trace.father != nodes[v]) {
            trace = trace.father;
        }

        nodes[v].childCnt -= trace.childCnt;

        total -= (long) nodes[u].childCnt * (long)nodes[v].childCnt;
