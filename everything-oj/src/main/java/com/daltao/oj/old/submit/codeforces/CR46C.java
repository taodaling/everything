...
public class CR46C {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        long end = 0;
        if (local) {
            io = new IOUtil(new FileInputStream("E:\\DATABASE\\TESTCASE\\CR46C.in"), new FileOutputStream("E:\\DATABASE\\TESTCASE\\CR46C.out"));
    ...
    public static class Task implements Runnable {
        ...
        public void run() {
            int n = io.readInt();
            long[][] sides = new long[n + 1][2];

            for (int i = 0; i < n; i++) {
                sides[i][0] = io.readLong();
                sides[i][1] = io.readLong() + 1;
            }
            sides[n][0] = Long.MAX_VALUE;
            sides[n][1] = Long.MAX_VALUE;

            long[][] leftOrder = sides.clone();
            long[][] rightOrder = sides.clone();

            Arrays.sort(leftOrder, new Comparator<long[]>() {
                @Override
            ...
            Arrays.sort(rightOrder, new Comparator<long[]>() {
                @Override
                public int compare(long[] o1, long[] o2) {
            ...
            long last = 0;
            while (l != n || r != n) {
                long min = Math.min(leftOrder[l][0], rightOrder[r][1]);
                cnts[cnt] += min - last;
                last = min;

                while (leftOrder[l][0] == min) {
                    cnt++;
                    l++;
                }
                while (rightOrder[r][1] == min) {
                    cnt--;
                    r++;
                }
            }
