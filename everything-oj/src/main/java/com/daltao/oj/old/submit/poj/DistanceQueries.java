...
public class DistanceQueries {
    ...
    public static void main(String[] args) throws IOException {
        while (!input.isMeetBlankAndEOF()) {
            DistanceQueries solution = new DistanceQueries();
            solution.init();
            solution.solve();
        }
    }
    ...
    public void init() throws IOException {
        ...
        for (int i = 0; i < edgeNum; i++) {
            Node src = nodeList.get(input.nextInteger() - 1);
            Node dst = nodeList.get(input.nextInteger() - 1);
        ...
        for (int i = 0; i < questionNum; i++) {
            ...
            question.dst = dst;
            if (src != dst) {
                src.questionList.add(question);
                dst.questionList.add(question);
            } else {
                question.answer = 0;
            }
    ...
    public void lca(Node node, Node prt, int depth) {
        if (node.visited) {
            return;
        }
        ...
        for (Edge edge : node.edgeList) {
            ...
            lca(sub, node, edge.weight + depth);
            node.union(sub);
            node.getRepr().ancestor = node;
        }

        node.visited = true;
        for (Question question : node.questionList) {
            Node other = question.src;
            if (other == node) {
                other = question.dst;
            }
            if (!other.visited) {
            ...
            question.answer = node.depth + other.depth - 2 * other.getRepr().ancestor.depth;
        }
    }

    public void solve() {
        lca(nodeList.get(0), null, 0);

        StringBuilder result = new StringBuilder();
        for (Question question : questionList) {
            result.append(question.answer).append('\n');
        }
    ...
    private static class Node {
        ...
        public void union(Node other) {
            union1(getRepr(), other.getRepr());
        }

        private static void union0(Node a, Node b) {
            if (a == b) {
                return;
        ...
        private static void union1(Node a, Node b) {
            b.p = a;
