...
public class ECR44D {
    ...
    public static void init() throws FileNotFoundException {
        if (System.getProperty("ONLINE_JUDGE") == null) {
            input = new BlockReader(new FileInputStream("E:\\DATABASE\\TESTCASE\\codeforces\\ECR44D.in"));
            output = System.out;
        } else {
    ...
    public static void solve() {
        ...
        long H = Long.parseLong(input.nextBlock());
        ...
        long remain = n - (l + 1) * l / 2;
        if (l > H) {
            remain += (l - H) * (l - H + 1) / 2;
        }
        long total = l + (remain + H - 1) / H;
        ...
        if (l >= H) {
            ...
            remain = n - ((l + 1) * l - l - H * (H - 1) / 2);
            long total2 = l + (l - H) + (remain + l - 1) / l;

            output.println(Math.min(total, total2));
        } else {
