...
public class ECR44C {
    ...
    public static void init() throws FileNotFoundException {
        if (System.getProperty("ONLINE_JUDGE") == null) {
            input = new BlockReader(new FileInputStream("E:\\DATABASE\\TESTCASE\\codeforces\\ECR44C.in"));
            output = System.out;
        } else {
    ...
    public static void solve() {
        ...
        int total = n * k;
        int[] staves = new int[total];
        for (int i = 0; i < total; i++) {
            staves[i] = input.nextInteger();
        }
        ...
        int firstBarrel = staves[0];
        int noMoreThan = firstBarrel + l;

        int cnt = 0;
        for (int i = 0; i < total; i++) {
            if (staves[i] <= noMoreThan) {
                cnt++;
            }
        }
        ...
        long sum = 0;
        int moreThan = total - cnt;
        int lessThan = cnt;
        for (int i = 0; i < n; i++) {
            int provide = 0;
            if (moreThan > 0) {
                provide = Math.min(moreThan, k - 1);
                moreThan -= provide;
            }
            lessThan -= k - provide;
            sum += staves[lessThan];
