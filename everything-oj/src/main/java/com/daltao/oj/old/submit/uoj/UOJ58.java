...
public class UOJ58 {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        long end = 0;
        if (local) {
            io = new IOUtil(new FileInputStream("E:\\DATABASE\\TESTCASE\\UOJ58.in"), new FileOutputStream("E:\\DATABASE\\TESTCASE\\UOJ58.out"));
    ...
    public static class Task implements Runnable {
        ...
        public void run() {
            ...
            for (int i = 1; i <= n; i++) {
                nodes[i].c = io.readInt();
            }

            List<Node> trace = new ArrayList<>(2 * n);
            dfs(nodes[1], null, trace);
            St<Node> st = new St<>(trace.toArray(), trace.size(), new Comparator<Node>() {
                @Override
            ...
            Counter counter = new Counter(m + 1);
            for (int i = 0; i < q; i++) {
                int t = io.readInt();
                int x = io.readInt();
                int y = io.readInt();
                ...
                if (t == 0) {
                    ...
                    Node a = nodes[x];
                    Node b = nodes[y];
                    Node lca = st.query(Math.min(a.order, b.order), Math.max(a.order, b.order));
                    counter.nextVersion();
                    long val = count(counter, a, lca, v, w)
                            + count(counter, b, lca.father, v, w);
                    io.write(val);
                    io.write('\n');
                }
            }
        ...
        static void dfs(Node root, Node father, List<Node> trace) {
            ...
            for (Node neighbor : root.neighbors) {
                ...
                dfs(neighbor, root, trace);
                trace.add(root);
            }
        }

        static long count(Counter cnt, Node from, Node end, int[] v, int[] w) {
            long total = 0;
            while (from != end) {
                cnt.mod(from.c, 1);
                total += (long) v[from.c] * w[cnt.get(from.c)];
                from = from.father;
            }
            return total;
        }
