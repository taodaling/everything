...
public class CF963A {
    ...
    public static void init() throws FileNotFoundException {
        if (System.getProperty("ONLINE_JUDGE") == null) {
            input = new BlockReader(new FileInputStream("E:\\DATABASE\\TESTCASE\\codeforces\\CF963A.in"));
            output = System.out;
        } else {
    ...
    public static void solve() {
        ...
        long an = pow(a, n);
        long inv_a = pow(a, MOD - 2);
        long b_div_a = b * inv_a % MOD;

        char[] s = new char[k];
        input.nextBlock(s, 0);

        long[] v = new long[k + 1];
        v[1] = (s[0] == '+' ? 1 : (MOD - 1)) * an % MOD;
        long aggre = an;
        for (int i = 1; i < k; i++) {
            long si = s[i] == '+' ? 1 : (MOD - 1);
            aggre = aggre * b_div_a % MOD;
            v[i + 1] = (aggre * si + v[i]) % MOD;
        }


        int m = (n + 1) / k;

        long t = pow(b_div_a, k);
        ...
        if (t == 1) {
            ...
            sn = (pow(t, m) + MOD - 1) % MOD * pow((t - 1 + MOD) % MOD, MOD - 2) % MOD;
        }
