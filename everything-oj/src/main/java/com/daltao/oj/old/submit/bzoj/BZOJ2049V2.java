...
public class BZOJ2049V2 {
    ...
    public static void init() throws FileNotFoundException {
        if (System.getProperty("ONLINE_JUDGE") == null) {
            input = new BlockReader(new FileInputStream("E:\\DATABASE\\TESTCASE\\bzoj\\BZOJ2049.in"));
            output = System.out;
        } else {
    ...
    public static void solve() {
        ...
        for (int i = 0; i < m; i++) {
            ...
            LCTNode u = nodes[input.nextInteger()];
            LCTNode v = nodes[input.nextInteger()];

            if (buf[0] == 'Q') {
                LCTNode.access(u);
                LCTNode.splay(u);
                u = LCTNode.findRoot(u);
                LCTNode.access(v);
                LCTNode.splay(v);
                v = LCTNode.findRoot(v);
                output.println(u == v ? "Yes" : "No");
            } else if (buf[0] == 'C') {
                LCTNode.join(u, v);
            } else {
                LCTNode.cut(u, v);
