...
public class Contest487A {
    public static void main(String[] args) throws FileNotFoundException {
        boolean local = System.getProperty("ONLINE_JUDGE") == null;
        IOUtil io;
        if (local) {
            io = new IOUtil(new FileInputStream("E:\\DATABASE\\TESTCASE\\codeforces\\Contest487A.in"), new FileOutputStream("E:\\DATABASE\\TESTCASE\\codeforces\\Contest487A.out"));
        } else {
        ...
        public void run() {
            ...
            for(int i = 1; i < n - 1; i++)
            {
                if(s.charAt(i) != '.' && s.charAt(i - 1) != '.' && s.charAt(i + 1) != '.'
                        && s.charAt(i) != s.charAt(i - 1) && s.charAt(i - 1) != s.charAt(i + 1) && s.charAt(i) != s.charAt(i + 1))
                {
                    io.write("Yes");
                    return;
                }
