...
public class ECR44A {
    ...
    public static void init() throws FileNotFoundException {
        if (System.getProperty("ONLINE_JUDGE") == null) {
            input = new BlockReader(new FileInputStream("E:\\DATABASE\\TESTCASE\\codeforces\\ECR44A.in"));
            output = System.out;
        } else {
    ...
    public static void solve() {
        ...
        int half = n / 2;
        int[] cell = new int[half];
        for (int i = 0; i < half; i++) {
            cell[i] = input.nextInteger();
        }
        ...
        for (int i = 0; i < half; i++) {
            int pos1 = i * 2 + 1;
            int pos2 = i * 2 + 2;
            cnt1 += Math.abs(cell[i] - pos1);
            cnt2 += Math.abs(cell[i] - pos2);
        }

        output.println(Math.min(cnt1, cnt2));
    }
