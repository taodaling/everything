...
public class CF961G {
    ...
    public static void init() throws FileNotFoundException {
        if (System.getProperty("ONLINE_JUDGE") == null) {
            input = new BlockReader(new FileInputStream("D:\\DataBase\\TESTCASE\\codeforces\\CF961G.in"));
            output = System.out;
        } else {
    ...
    public static void solve() {
        ...
        long w = 0;
        for (int i = 1; i <= n; i++) {
            w = w + input.nextInteger();
        }
        w %= MOD;

        long result = (stirling(n, m) + (long) (n - 1) * stirling(n - 1, m) % MOD) % MOD * w % MOD;
    ...
    public static int stirling(int n, int m) {
        ...
        for (int i = 0; i <= m; i++) {
            ...
            sum = (sum + fac * top % MOD * pow((int) low, MOD - 2) % MOD * pow(m - i, n)) % MOD;

        }

        sum = sum * pow((int) top, MOD - 2) % MOD;
