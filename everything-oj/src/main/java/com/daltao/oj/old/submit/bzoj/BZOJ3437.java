    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 1; i <= n; i++) {
                ...
                node.s = deque.peekLast().s + count[i];
                node.st = deque.peekLast().st + (long) count[i] * i;
                while (deque.size() > 1) {
                    Node head = deque.removeFirst();
                    if (deque.peekFirst().g > i) {
                        deque.addFirst(head);
                        break;
                    }
                }
                Node support = deque.peekFirst();
                node.dp = support.dp + i * (node.s - support.s) - (node.st - support.st) + fee[i];
                node.x = node.s;
                node.y = node.dp + node.st;
                while (!deque.isEmpty()) {
