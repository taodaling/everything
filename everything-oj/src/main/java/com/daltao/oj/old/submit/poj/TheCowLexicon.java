...
public class TheCowLexicon {
    ...
    public static void main(String[] args) throws IOException {
        while (!input.isMeetBlankAndEOF()) {
            TheCowLexicon cowLexicon = new TheCowLexicon();
            cowLexicon.init();
            System.out.println(cowLexicon.solve());
        }
    }

    public void init() throws IOException {
        dictSize = input.nextInteger();
        msgLength = input.nextInteger();
        msg = input.nextBlock().toCharArray();

        dict = new char[dictSize][];
        for (int i = 0; i < dictSize; i++) {
            dict[i] = input.nextBlock().toCharArray();
        }
    }

    public int solve() {
        int[] dp = new int[msgLength];
        int formerMin = msgLength + 1;
        for (int i = 0; i < msgLength; i++) {
            dp[i] = formerMin - (msgLength - i);
            for (char[] word : dict) {
                int j = i;
                int w = word.length - 1;
                while (j >= 0 && w >= 0) {
                ...
                if (w < 0) {
                    //Find a match
                    dp[i] = Math.min(dp[i], (j < 0 ? 0 : dp[j]) + (i - j) - (word.length - 1 - w));
                }
            }
            formerMin = Math.min(formerMin, dp[i] + msgLength - i);
        }

        return dp[msgLength - 1];
    }
