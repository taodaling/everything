...
public class Contest480F {
    ...
    public static void init() throws FileNotFoundException {
        ...
        if (System.getProperty("ONLINE_JUDGE") == null) {
            input = new BlockReader(new FileInputStream("E:\\DATABASE\\TESTCASE\\codeforces\\Contest480F.in"));
            output = System.out;
        } else {
    ...
    public static void solve() {
        ...
        dfs(nodes[n], nodes[0]);
        PriorityQueue<Node> nodePriorityQueue = new PriorityQueue<>(new Comparator<Node>() {
            @Override
        ...
        for (int i = 1; i <= n; i++) {
            if (nodes[i].childrenCnt == 0) {
                nodePriorityQueue.add(nodes[i]);
            }
        }

        int[] removed = new int[k];
        for (int i = 0; i < k; i++) {
            Node head = nodePriorityQueue.remove();
            removed[i] = head.id;

            head.parent.childrenCnt--;
            if (head.parent.childrenCnt == 0) {
                nodePriorityQueue.add(head.parent);
            }
        }

        Arrays.sort(removed);
        for (int i = 0; i < k; i++) {
            output.print(removed[i]);
            output.print(' ');
        }
