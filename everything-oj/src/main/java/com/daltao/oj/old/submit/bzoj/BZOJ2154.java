...
public class BZOJ2154 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            int[] minFactorExp = new int[limit + 1];
            int[] minFactor = new int[limit + 1];
            int[] sigma = new int[limit + 1];
            int mod = 20101009;
            mu[1] = 1;
            sigma[1] = 1;
            int wpos = 0;
            for (int i = 2; i <= limit; i++) {
                if (!isComp[i]) {
                    primes[wpos++] = i;
                    mu[i] = -1;
                    sigma[i] = (sigma[1] + i * mu[i]) % mod;
                    minFactor[i] = i;
                    minFactorExp[i] = i;
                } else {
                    mu[i] = mu[minFactorExp[i]] * mu[i / minFactorExp[i]];
                    sigma[i] = (int) ((sigma[i / minFactorExp[i]] + (long) sigma[i / minFactorExp[i]] * minFactor[i] * (-1)) % mod);
                }
                if (sigma[i] < 0) {
                    sigma[i] += mod;
                }
                ...
                for (int j = 0, until = limit / i; j < wpos && primes[j] <= until; j++) {
                    int pi = primes[j] * i;
                    isComp[pi] = true;
                    minFactor[pi] = primes[j];
                    minFactorExp[pi] = i % primes[j] != 0 ? primes[j] : (primes[j] * minFactorExp[i]);
                    if (i % primes[j] == 0) {
                        break;
                    }
                }
            ...
            int[] prefixSumOfSigma = new int[limit + 1];
            for (int i = 1; i <= limit; i++) {
                prefixSumOfSigma[i] = (int) ((prefixSumOfSigma[i - 1] + (long) sigma[i] * i) % mod);
            }

            int sum = 0;
            for (int i = 1, r; i <= n; i = r + 1) {
                r = Math.min(n / (n / i), m / (m / i));
                sum += (long)(prefixSumOfSigma[r] - prefixSumOfSigma[i - 1])
                        * calcSum(n / i, mod) % mod * calcSum(m / i, mod) % mod;
                sum %= mod;
                if (sum < 0) {
                    sum += mod;
                }
        ...
        public static int calcSum(int n, int mod) {
