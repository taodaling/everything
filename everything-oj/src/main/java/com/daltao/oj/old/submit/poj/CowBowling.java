...
public class CowBowling {
    ...
    public static void main(String[] args) throws IOException {
        while (!input.isMeetBlankAndEOF()) {
            CowBowling cowBowling = new CowBowling();
            cowBowling.init();
            System.out.println(cowBowling.solve());
        }
    }
    ...
    public void init() throws IOException {
        n = input.nextInteger();
        tree = new int[(1 + n) * n / 2];
        for (int i = 0, bound = tree.length; i < bound; i++) {
            tree[i] = input.nextInteger();
        }
    }
    ...
    public int solve() {
        ...
        for (int j = 0; j < n; j++) {
            int index = getIndex(n - 1, j);
            dp[index] = tree[index];
        }
        for (int i = n - 2; i >= 0; i--) {
            for (int j = 0; j <= i; j++) {
                int index = getIndex(i, j);
                dp[index] = tree[index] + Math.max(dp[getIndex(i + 1, j)], dp[getIndex(i + 1, j + 1)]);
            }
        }
