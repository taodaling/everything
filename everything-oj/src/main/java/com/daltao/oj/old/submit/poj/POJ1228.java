...
public class POJ1228 {
    public static void main(String[] args) throws FileNotFoundException {
        boolean local = System.getProperty("ONLINE_JUDGE") == null;
        IOUtil io;
        if (local) {
            io = new IOUtil(new FileInputStream("E:\\DATABASE\\TESTCASE\\POJ1228.in"), new FileOutputStream("E:\\DATABASE\\TESTCASE\\POJ1228.out"));
        } else {
    ...
    public static class Task implements Runnable {
        ...
        public void solveOnce() {
            ...
            List<Geometry.Point2> list = new ArrayList(n);
            for (int i = 0; i < n; i++) {
                list.add(new Geometry.Point2(io.readInt(), io.readInt()));
            }
            ...
            if (!Geometry.allOnEdge(list, 1e-6)) {
                io.write("NO\n");
                return;
            }
    ...
    public static class Geometry {
        ...
        public static boolean allOnEdge(List<Point2> point2s, final double prec) {
            final Point2[] points = point2s.toArray(new Point2[0]);
            int n = points.length;
            ...
            HashMap<Point2, Integer> map = new HashMap(n);
            Arrays.sort(points, 1, n, cmp);

            boolean valid = false;
            int shrinkSize = 2;
            for (; n >= 2; n--) {
                if (cmp.compare(points[n - 1], points[n - 2]) == 0) {
                    valid = true;
                    if (distance2(points[n - 1].sub(points[0])) > distance2(points[n - 2].sub(points[0]))) {
            ...
            for (int i = 1; i < n; i++) {
                while (stack.size() >= 2) {
                    Point2 last = stack.removeLast();
                    Point2 second = stack.peekLast();
                    double c = cross(points[i].sub(second), last.sub(second));
                    if (c < -prec) {
                        stack.addLast(last);
                        break;
                    } else if (Math.abs(c) > prec) {
                        return false;
                    }
                }
                stack.addLast(points[i]);
            ...
            Point2[] pts = stack.toArray(new Point2[0]);
            for (int i = 1, until = pts.length - 1; i < until; i++) {
                if (map.get(pts[i]) + 1 == map.get(pts[i + 1])) {
                    return false;
                }
            }
