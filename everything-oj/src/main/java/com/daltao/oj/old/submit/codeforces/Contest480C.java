...
public class Contest480C {
    ...
    public static void init() throws FileNotFoundException {
        if (System.getProperty("ONLINE_JUDGE") == null) {
            input = new BlockReader(new FileInputStream("E:\\DATABASE\\TESTCASE\\codeforces\\Contest480C.in"));
            output = System.out;
        } else {
    ...
    public static void solve() {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (group[color] == -1) {
                int j = color - 1;
                int until = Math.max(0, color - k + 1);
                for (; j >= until; j--) {
                    if (group[j] != -1) {
                        break;
                    }
                ...
                for (int t = group[color]; t < color; t++) {
                    group[t] = group[color];
                }
            }
            output.print(group[color]);
            output.print(' ');
        }
    }
