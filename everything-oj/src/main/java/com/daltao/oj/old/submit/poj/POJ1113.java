...
public class POJ1113 {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        IOUtil io;
        if (local) {
            io = new IOUtil(new FileInputStream("E:\\DATABASE\\TESTCASE\\poj\\POJ1113.in"), new FileOutputStream("E:\\DATABASE\\TESTCASE\\poj\\POJ1113.out"));
        } else {
    ...
    public static class Task implements Runnable {
        ...
        public void run() {
            ...
            for (int i = 0; i < n; i++) {
                points[i] = new Geometry.Point2(io.readInt(), io.readInt());
            }

            List<Geometry.Point2> point2List = Geometry.grahamScan(Arrays.asList(points), 1e-3);
            n = point2List.size();
            double length = Geometry.distance(point2List.get(n - 1).sub(point2List.get(0)));
            for (int i = 1; i < n; i++) {
                length += Geometry.distance(point2List.get(i).sub(point2List.get(i - 1)));
            }

            double radian = 2 * Math.PI;
            double circle = radian * l;

            io.write(Mathematics.longRound(length + circle));
        }
    }
