...
public class CashMachine {
    ...
    public static void main(String[] args) throws IOException {
        while (!input.isMeetBlankAndEOF()) {
            ...
            cashMachine.init();
            System.out.println(cashMachine.solve());
        }
    }
    ...
    int denominationNum;
    int[] denominations;
    int[] denominationRemainNums;

    public void init() throws IOException {
        requireAmount = input.nextInteger();
        denominationNum = input.nextInteger();

        denominationRemainNums = new int[denominationNum];
        denominations = new int[denominationNum];

        for (int i = 0; i < denominationNum; i++) {
            denominationRemainNums[i] = input.nextInteger();
            denominations[i] = input.nextInteger();
        }
    }

    public int solve() {
        int[] dp = new int[requireAmount + 1];
        for (int i = 0; i < denominationNum; i++) {
            int denomination = denominations[i];
            int remain = denominationRemainNums[i];
            for (int j = 0; j < denomination; j++) {
                MaxQueue queue = new MaxQueue(remain + 1, remain + 1);
                for (int k = j; k <= requireAmount; k += denomination) {
                    queue.deque();
                    queue.enque(dp[k] - (k - j) + requireAmount);
                    dp[k] = queue.max() + (k - j) - requireAmount;
                }
            }
        }

        return dp[requireAmount];
    }
    ...
    private static class IntQueue {
        ...
        public IntQueue(int cap, int addZeroNum) {
            this.cap = cap + 1;
        ...
        public int peekLast() {
            return queue[(tail > 0 ? tail : cap) - 1];
        }

        public int popLast() {
            tail = (tail > 0 ? tail : cap) - 1;
            return queue[tail];
        }
    }

    private static class MaxQueue {
        private IntQueue decrease;
        private IntQueue queue;

        public MaxQueue(int cap, int addZeroNum) {
            decrease = new IntQueue(cap, addZeroNum);
            queue = new IntQueue(cap, addZeroNum);
        }
        ...
        public int deque() {
            int ret = queue.deque();
            if (!decrease.isEmpty() && decrease.peek() == ret) {
                decrease.deque();
            }
            return ret;
        }
