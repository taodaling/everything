...
public class BZOJ2152 {
    public static void main(String[] args) throws FileNotFoundException {
        boolean local = System.getProperty("ONLINE_JUDGE") == null;
        IOUtil io;
        if (local) {
            io = new IOUtil(new FileInputStream("E:\\DATABASE\\TESTCASE\\bzoj\\BZOJ2152.in"), new FileOutputStream("E:\\DATABASE\\TESTCASE\\bzoj\\BZOJ2152.out"));
        } else {
    ...
    public static class Task implements Runnable {
        ...
        public void run() {
            ...
            for (int i = 1; i < n; i++) {
                Edge edge = new Edge();
                edge.src = nodes[io.readInt()];
                edge.dst = nodes[io.readInt()];
                edge.num = Mathematics.mod(io.readInt(), 3);

                edge.src.children.add(edge);
                edge.dst.children.add(edge);
            }
            int[] cnt = new int[3];

            Deque<Node> queue = new ArrayDeque(n);
            Deque<Node> stack = new ArrayDeque(n);
            queue.addLast(nodes[1]);
            nodes[1].depth = 0;
            while (!queue.isEmpty()) {
                Node head = queue.removeFirst();
                stack.addLast(head);
                for (Edge edge : head.children) {
                    Node other = edge.getOther(head);
                    if (other.depth != -1) {
                        continue;
                    }
                    other.depth = head.depth + 1;
                    queue.addLast(other);
                }
            }
            while (!stack.isEmpty()) {
                ...
                last.dp[0] = 1;
                cnt[0]++;
                for (Edge edge : last.children) {
                    Node other = edge.getOther(last);
                    if (other.depth <= last.depth) {
                        continue;
                    }
                    ...
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 3; j++) {
                            cnt[(i + j + edge.num) % 3] += 2 * last.dp[i] * other.dp[j];
                        }
                    }
                    for (int i = 0; i < 3; i++) {
                        last.dp[(i + edge.num) % 3] += other.dp[i];
                    }
                }
            }
            ...
            int g = Mathematics.gcd(cnt[0], allChoose);
            io.write(cnt[0] / g);
            io.write('/');
            io.write(allChoose / g);
        }
    }
