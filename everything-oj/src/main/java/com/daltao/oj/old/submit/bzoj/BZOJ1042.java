...
public class BZOJ1042 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            int total = io.readInt();

            //dp[i][j] means how many way to form total with the first i type coins
            int sLimit = 100000;
            long[][] dp = new long[4][sLimit + 1];
            for (int i = 0; i < 4; i++) {
                dp[i][0] = 1;
                for (int j = 1; j <= sLimit; j++) {
                    if (i > 0) {
                        dp[i][j] = dp[i - 1][j];
                    } else {
                        dp[i][j] = 0;
            ...
            for (int i = 0; i < total; i++) {
                for (int j = 0; j < 4; j++) {
                    limit[j] = io.readInt() + 1;
                }
                int s = io.readInt();
                long possible = -composite(coins, limit, 0, s, dp[3], 0);
                io.cache.append(possible).append('\n');
            }
        }

        public long composite(int[] coins, int[] limit, int i, int total, long[] dp, int time) {
            if (total < 0) {
                return 0;
            }
            if (i == coins.length) {
                ...
                return dp[total] * ((time % 2) * 2 - 1);
            }
            return composite(coins, limit, i + 1, total, dp, time)
                    + composite(coins, limit, i + 1, total - limit[i] * coins[i], dp, time + 1);
        }
    }
