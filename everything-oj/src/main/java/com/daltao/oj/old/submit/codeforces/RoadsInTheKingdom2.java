...
public class RoadsInTheKingdom2 {
    ...
    boolean searchLoop(Node node, Node from)
    {
        if (node.flag == Node.FLAG_VISITED) {
            ...
            //Remove (from, node)
            long distance = node.distanceTo(from);
            node.removeNearNode(from);
    ...
    void preCalculateEndAndMid(Node node, Node from) {
        ...
        for (Node next : node.nearNodes) {
            ...
            maxMid = maxMid > next.mid ? maxMid : next.mid;
        }
    ...
    long solve() {
        ...
        long[] right = new long[loopSize];
        {
            ...
            for (int i = 1, bound = loopSize; i < bound; i++) {
                left[i] = Math.max(left[i - 1], lm[i] + w[i]);
            }
            right[loopSize - 1] = INFINITE_SMALL / 4;
            for (int i = loopSize - 2; i >= 0; i--) {
        ...
        for (int i = 0, bound = loopSize - 1; i < bound; i++) {
            ...
            inconvenience = Math.max(left[i], possibleResult);
            inconvenience = Math.max(right[i], possibleResult);
            inconvenience = Math.max(cross[i], possibleResult);
