...
public class SetTheory {
    public static void main(String[] args) throws Exception {
        ...
        int[] forbiden = new int[1000000];
        Set<Integer> differSet = new TreeSet<>();
        StringBuilder resultBuilder = new StringBuilder();
        while (testCaseNum > 0) {
            differSet.clear();
            int aSize = input.nextInteger();
            int[] a = new int[aSize];
            for (int i = 0; i < aSize; i++) {
                a[i] = input.nextInteger();
                for (int j = 0; j < i; j++) {
                    differSet.add(Math.abs(a[i] - a[j]));
                }
            }
            int[] differArray = new int[differSet.size()];
            {
                int wpos = 0;
                for (Integer val : differSet) {
                    differArray[wpos++] = val;
                }
            }
            ...
            for (int i = 1; i <= 1000000; i++) {
                ...
                for (int val : differArray) {
                    forbiden[i + val] = testCaseNum;
                }
            }
