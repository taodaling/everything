...
public class LUOGU3431 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            int[] allValues = new int[k];
            int[][] passengers = new int[k][3];
            for (int i = 0; i < k; i++) {
                int x = io.readInt();
                int y = io.readInt();
                int p = io.readInt();
            ...
            Arrays.sort(passengers, (a, b) -> a[1] == b[1] ? a[0] - b[0] : a[1] - b[1]);

            for (int[] p : passengers) {
                long dp = root.query(map.minRank(), p[0], map.minRank(), map.maxRank())
                        + p[2];
                root.update(p[0], p[0], map.minRank(), map.maxRank(), dp);
            }

            long ans = root.query(map.minRank(), map.maxRank(), map.minRank(), map.maxRank());
            io.cache.append(ans);
        }
    }
