...
public class CF1168E {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            io.cache.append("Shi\n");
            for (int i = 0; i < n; i++) {
                io.cache.append(helper.p[i]).append(" ");
            }
            io.cache.append('\n');
            for (int i = 0; i < n; i++) {
                io.cache.append(helper.q[i]).append(" ");
            }
        }
    }
    ...
    public static class Helper {
        ...
        public void xor(int i, int j, int x) {
            a[i] ^= x;
            a[j] ^= x;
            fix(i, j);
        }
        ...
        public void swapP(int i, int j) {
            Memory.swap(p, i, j);
            Memory.swap(pIndex, p[i], p[j]);
        }

        public void fix(int i, int j) {
            if (a[i] == (p[i] ^ q[i])) {
                return;
            }
            int t = pIndex[a[i] ^ q[i]];
            swapP(i, t);
            Memory.swap(q, j, t);
            fix(t, j);
        }
        ...
        public String toString() {
            ...
            for (int i = 0; i < a.length; i++) {
                builder.append(p[i]).append(" ").append(q[i]).append(" ").append(a[i]).append('\n');
            }
