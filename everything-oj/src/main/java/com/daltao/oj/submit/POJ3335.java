...
public class POJ3335 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int n = io.readInt();
            GeometryUtils.Point2D[] pts = new GeometryUtils.Point2D[n];
            for (int i = 0; i < n; i++) {
                pts[i] = new GeometryUtils.Point2D(io.readInt(), io.readInt());
            }
            Memory.reverse(pts, 0, n);
            List<GeometryUtils.Line2D> lines = new ArrayList(n);
            lines.add(new GeometryUtils.Line2D(pts[n - 1], pts[0]));
            for (int i = 1; i < n; i++) {
                lines.add(new GeometryUtils.Line2D(pts[i - 1], pts[i]));
            }
            GeometryUtils.HalfPlaneIntersection hpi = new GeometryUtils.HalfPlaneIntersection(lines, true);
            io.cache.append(hpi.convex == null ? "NO\n" : "YES\n");
            return;
        }
    }
    ...
    public static class GeometryUtils {
        ...
        public static class HalfPlaneIntersection {
            ...
            public HalfPlaneIntersection(List<Line2D> lineList, boolean close) {
                Line2D[] lines = lineList.toArray(new Line2D[lineList.size() + 4]);
                lines[lineList.size()] = new Line2D(new Point2D(-INF, -INF), new Point2D(INF, -INF));
                lines[lineList.size() + 1] = new Line2D(new Point2D(INF, -INF), new Point2D(INF, INF));
                lines[lineList.size() + 2] = new Line2D(new Point2D(INF, INF), new Point2D(-INF, INF));
                lines[lineList.size() + 3] = new Line2D(new Point2D(-INF, INF), new Point2D(-INF, -INF));
                ...
                List<Line2D> convexEdges = new ArrayList();
                while (!deque.isEmpty()) {
                    Line2D head = deque.removeFirst();
                    if (head.a.x == INF || head.a.x == -INF) {
                        continue;
                    }
                    convexEdges.add(head);
                }
                convex = new ConvexHull(convexEdges);
            }
