...
public class LUOGU1337 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int n = io.readInt();
            double[][] balls = new double[n][3];
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < 3; j++) {
                    balls[i][j] = io.readInt();
                }
            }

            SimulatedAnnealing<double[]> sa = new SimulatedAnnealing<double[]>(1e-6, 1e-50, 0.998) {
                Map<double[], double[]> map = new HashMap<>();

                @Override
                public double[] next(double[] old, double temperature) {
//                    double[] direction = map.get(old);
//                    double dist = nextDouble(0, temperature);
                    double[] next = new double[]{
                            old[0] + nextDouble(-temperature, temperature),//direction[0] * dist,
                            old[1] + nextDouble(-temperature, temperature)//direction[1] * dist
                    };
                    return next;
                }
                ...
                public double eval(double[] status) {
                    ...
                    for (double[] ball : balls) {
                        double dx = ball[0] - status[0];
                        double dy = ball[1] - status[1];
...
//                        d = 1e-30;
//                    }
//                    map.put(status, new double[]{x / d, y / d});
                    return -(x * x + y * y);
                }
            };

            for (int i = 0; i < 20; i++) {
                sa.optimize(5000, new double[]{sa.nextDouble(-10000, 10000), sa.nextDouble(-10000, 10000)});
            }
            double[] xy = sa.getBest();
            io.cache.append(String.format("%.3f %.3f", xy[0], xy[1]));
        }
    }
