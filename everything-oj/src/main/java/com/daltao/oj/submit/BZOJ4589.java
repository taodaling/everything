...
public class BZOJ4589 {
    ...
    public static class Task implements Runnable {
        ...
        int mod = (int) 1e9 + 7;
        int limit = 1 << 16;
        long inv2 = Mathematics.inverse(2, mod);
        int[] standard;
        int[] dp0 = new int[limit];
        int[] base = new int[limit];

        private void initStandard() {
            int[] primes = new int[limit];
            int[] primeRecord = new int[limit];
            Arrays.fill(primes, 2, limit, 1);
            int primeCount = 0;
            for (int i = 2; i < limit; i++) {
                if (primes[i] == 1) {
                    primeRecord[primeCount++] = i;
                }
                for (int j = 0; j < primeCount && i * primeRecord[j] < limit; j++) {
                    int pi = i * primeRecord[j];
                    primes[pi] = 0;
                    if (i % primeRecord[j] == 0) {
                        break;
                    }
                }
            }
        ...
        public void run() {
            if (debug.allowDebug) {
                int[] p = new int[]{1, 2};
                ftw(p, 0, 1);
                debug.assertTrue(p[0] == 3);
                debug.assertTrue(p[1] == mod - 1);
                iftw(p, 0, 1);
                debug.assertTrue(p[0] == 1);
                debug.assertTrue(p[1] == 2);
            }
        ...
        public void solve() {
            ...
            System.arraycopy(standard, 0, base, 0, m + 1);
            int properLength = 1 << Mathematics.ceilPowerOf2(m + 1);
            Arrays.fill(base, m + 1, properLength, 0);
            Arrays.fill(dp0, 0, properLength, 0);
            dp0[0] = 1;

            ftw(dp0, 0, properLength - 1);
            ftw(base, 0, properLength - 1);
            pow(base, properLength, dp0, n);
            iftw(dp0, 0, properLength - 1);
            io.cache.append(dp0[0]).append('\n');
        }
        ...
        public void pow(int[] p, int length, int[] prod, int n) {
            int bit = Mathematics.floorPowerOf2(n);
            for (; bit >= 0; bit--) {
                dotMul(prod, prod, length);
                if (bitAt(n, bit) == 1) {
                    dotMul(p, prod, length);
                }
            }
        }

        public void dotMul(int[] p, int[] output, int length) {
            for (int i = 0; i < length; i++) {
                output[i] = mod((long) p[i] * output[i]);
            }
        }
        ...
        public void ftw(int[] p, int l, int r) {
            if (r == l) {
                return;
            }
            int m = (l + r) >> 1;
        ...
        public void iftw(int[] p, int l, int r) {
            if (r == l) {
                return;
            }
            int m = (l + r) >> 1;
