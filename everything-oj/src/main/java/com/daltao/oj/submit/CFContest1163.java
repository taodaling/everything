...
public class CFContest1163 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            final Comparator<Point> lineCmp = (a, b) -> {
                return Long.signum(-Geometry.cross(a.x, a.y, b.x, b.y));
            };

            Comparator<Point> cmp = (a, b) -> {
                int r = lineCmp.compare(a, b);
                if (r == 0) {
                    r = Long.signum(-Geometry.cross(a.x0 - b.x0, a.y0 - b.y0, a.x, a.y));
                }
                return r;
            };

            TreeSet<Point> set = new TreeSet<>(cmp);
            for (int i = 0; i < n; i++) {
                for (int j = i + 1; j < n; j++) {
                    set.add(new Point(points[j][0] - points[i][0], points[j][1] - points[i][1], points[j][0], points[j][1]));
                }
            }
            ...
            for (; iterator.hasNext(); ) {
                ...
                i++;
                if (lineCmp.compare(last, pt) == 0) {
                    num++;
                    continue;
                }
                last = pt;
                cnt += num * (set.size() - i + 1);
                num = 1;
            }
            io.cache.append(cnt);
        }
    ...
    public static class Point {
        ...
        public Point(long x, long y, long x0, long y0) {
            if (y < 0) {
                y = -y;
                x = -x;
            }
