...
public class POJ2449 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            Node src = nodes[io.readInt()];
            Node dst = nodes[io.readInt()];
            int k = io.readInt();
            for (int i = 0; i <= n; i++) {
                nodes[i].h = inf;
            }
            ...
            for (int i = 1; i <= n; i++) {
                ...
                for (Edge edge : node.in) {
                    int newLen = edge.length + node.h;
                    if (newLen >= edge.src.h) {
                        continue;
                    }
                    edge.src.h = newLen;
                }
            ...
            TreeSet<Trace> set = new TreeSet<Trace>(new Comparator<Trace>() {
                @Override
                public int compare(Trace a, Trace b) {
                    int cp = a.g + a.current.h - b.g - b.current.h;
                    return cp != 0 ? cp : -1;
                }
            });
            set.add(new Trace(src, 0));
            while (set.size() > 0) {
                Trace trace = set.pollFirst();
                if (trace.current == dst) {
                    if (--hit == 0) {
                        io.cache.append(trace.g);
                        return;
                    }
                }
                for (Edge edge : trace.current.out) {
                    set.add(new Trace(edge.dst, trace.g + edge.length));
                }
            }
            io.cache.append("-1");
            return;
    ...
    public static class Trace {
        ...
        public Trace(Node current, int g) {
