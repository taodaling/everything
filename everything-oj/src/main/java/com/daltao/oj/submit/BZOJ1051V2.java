...
public class BZOJ1051V2 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 0; i < m; i++) {
                dcs.differenceLessThanOrEqualTo(io.readInt() - 1, io.readInt() - 1, 0);
            }
            if (!dcs.hasSolution()) {
            ...
            int maxIndex = 0;
            for (int i = 1; i < n; i++) {
                if (dcs.possibleSolutionOf(maxIndex) < dcs.possibleSolutionOf(i)) {
                    maxIndex = i;
                }
            }

            dcs.runSpfaSince(maxIndex);
            for (int i = 0; i < n; i++) {
                if (dcs.possibleSolutionOf(i) == DifferenceConstraintSystem.INF) {
                    io.cache.append(0);
                    return;
                }
            }
