...
public class BZOJ4700 {
    ...
    public static class Task implements Runnable {
        ...
        final Debug debug;
        int inf = (int) 1e8;
        LineOptimization optimization = new LineOptimization();
        ...
        public void solve() {
            ...
            for (int i = 0; i < n; i++) {
                ...
                nodes[i].t = (io.readInt() + atk - 1) / atk;
            }

            Arrays.sort(nodes, new Comparator<Node>() {
                @Override
                public int compare(Node o1, Node o2) {
                    int d = o2.a * o1.t - o1.a * o2.t;
                    if (d == 0) {
                        d = o2.t - o1.t;
                    }
                    return d;
            ...
            long sum = 0;
            for (int i = n - 2; i >= 0; i--) {
                nodes[i].s = nodes[i + 1].a + nodes[i + 1].s;
            }
            nodes[0].p = nodes[0].t - 1;
            sum += nodes[0].p * nodes[0].a;
            for (int i = 1; i < n; i++) {
                nodes[i].p = nodes[i - 1].p + nodes[i].t;
                sum += nodes[i].p * nodes[i].a;
            }

            add(nodes[0]);
            double max = 0;
            for (int i = 1; i < n; i++) {
                max = Math.max(optimization.query(nodes[i].a) + nodes[i].p * nodes[i].a + nodes[i].t * nodes[i].s, max);
                add(nodes[i]);
            }
        ...
        public void add(Node node) {
            optimization.insert(-node.t, node.p * node.a + node.t * node.s);
        }
    }
    ...
    public static class LineOptimization {
