    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 0; i < m; i++) {
                ...
                int c = edges[i][2];
                int w = edges[i][3];
                mcmf.getChannel(u, v, 0).modify(c, 0);
            }


            int maxflow = (int) (mcmf.send(inf)[0] + 0.5);
            for (int i = 0; i < m; i++) {
                int u = edges[i][0];
                int v = edges[i][1];
                int c = edges[i][2];
                int w = edges[i][3];
                mcmf.getChannel(u, v, w).modify(inf, 0);
            }

            int fee = (int) (mcmf.send(k)[1] + 0.5);

            io.cache.append(maxflow).append(' ').append(fee);
        }
    }
