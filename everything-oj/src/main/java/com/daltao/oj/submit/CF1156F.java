...
public class CF1156F {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            permutation[0] = 1;
            for (int i = 1; i <= n; i++) {
                permutation[i] = mod((long) permutation[i - 1] * i);
            }
            int total = permutation[n];
            ...
            for (int i = 1; i <= n; i++) {
                ...
                if (cnt[i] > 1) {
                    for (int j = 0; j + 2 <= n; j++) {
                        win = mod(win + (long) dp[j] * cnt[i] % mod * (cnt[i] - 1) % mod * permutation[n - j - 2] % mod);
                    }
                }
                for (int j = n - 1; j >= 1; j--) {
                    dp[j] = mod(dp[j] + (long)dp[j - 1] * cnt[i]);
                }
            }

            int probability =  mod((long)Mathematics.inverse(total, mod) * win);
            io.cache.append(probability);
        }
    }
