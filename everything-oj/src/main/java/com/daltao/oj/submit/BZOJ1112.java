    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = k; i <= n; i++) {
                ...
                //Find center
                root = Splay.selectKthAsRoot(root, (k + 1) / 2);
                root.pushDown();
                int key = root.key;
                long cost = (long) root.left.size * key -
                        root.left.sum
                        + root.right.sum - (long) root.right.size * key;
                minCost = Math.min(minCost, cost);

                //Remove first point
                root = Splay.selectKeyAsRoot(root, data[i - k + 1]);
                root = Splay.deleteRoot(root);
            }

            io.cache.append(minCost);
        }
    }
