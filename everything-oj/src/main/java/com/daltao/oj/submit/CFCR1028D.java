...
public class CFCR1028D {
    public static void main(String[] args) throws Exception {
        ...
        FastIO io = local ? new FastIO(new FileInputStream("/Users/daltao/DATABASE/TESTCASE/CFCR1028D.in"), System.out, charset) : new FastIO(System.in, System.out, charset);
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            Deque<Integer> unknown = new ArrayDeque<>(n);
            ...
            for (int i = 0; i < n; i++) {
                int minSell = sell.first();
                int maxBuy = buy.last();

                //ADD
                if (io.readString(command, 0) == 3) {
                    int p = io.readInt();
                    if (p > minSell) {
                        sell.add(p);
                    } else if (p < maxBuy) {
                ...
                //Accept
                    ...
                    while (!unknown.isEmpty()) {
                        Integer val = unknown.removeFirst();
                        if (val < p) {
                            buy.add(val);
                        } else if(val > p) {
                            sell.add(val);
            ...
            possible = (int) ((long)possible * (1 + unknown.size()) % MOD);
