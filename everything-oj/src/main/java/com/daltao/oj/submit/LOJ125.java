...
public class LOJ125 {
    ...
    public static class Task implements Runnable {
        ...
        MathUtils.InverseNumber in = new MathUtils.InverseNumber(10, modular);
        ...
        public void solve() {
            int n = io.readInt();
            MathUtils.FloorDivisionOptimizer optimizer = new MathUtils.FloorDivisionOptimizer(n, 1, n);
            int sum = 0;
            while (optimizer.hasNext()) {
                int f = optimizer.next();
                int addition = modular.plus(preSumOf(optimizer.r), -preSumOf(optimizer.l - 1));
                sum = modular.plus(sum, modular.mul(f, addition));
            }
            io.cache.append(sum);
        }

        public int preSumOf(int n) {
            int n1 = n;
            int n2 = modular.mul(n, n1);
            int n3 = modular.mul(n, n2);

            int sum0 = n;
            sum0 = modular.mul(sum0, 5);
            int sum1 = 9;
            sum1 = modular.plus(sum1, modular.mul(in.inv[2], n1));
            sum1 = modular.plus(sum1, modular.mul(in.inv[2], n2));
            sum1 = modular.mul(sum1, 3);
            int sum2 = 0;
            sum2 = modular.plus(sum2, modular.mul(in.inv[6], n1));
            sum2 = modular.plus(sum2, modular.mul(in.inv[2], n2));
            sum2 = modular.plus(sum2, modular.mul(in.inv[3], n3));
            sum2 = modular.mul(sum2, 2);

            return modular.plus(modular.plus(sum0, sum1), sum2);
        }
    }
