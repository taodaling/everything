...
public class LUOGU4548 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int n = io.readInt();
            int t = io.readInt();
            int[] pattern = new int[100001];
            Hash h31 = new Hash(100001, 31);
            Hash h61 = new Hash(100001, 61);
            NumberTheory.Modular mod = new NumberTheory.Modular(10000);

            for (int i = 0; i < t; i++) {
                int m = io.readInt();
                for (int j = 1; j <= m; j++) {
                    pattern[j] = io.readInt();
                }
                h31.populate(pattern, m + 1);
                h61.populate(pattern, m + 1);
                int sum = 0;
                int ni = 1;
                for (int j = 1; j <= m; j++) {
                    int ll = 1;
                    ...
                    int rr = m;
                    ni = mod.mul(ni, n);
                    if (h31.partial(ll, lr) == h31.partial(rl, rr) &&
                            h61.partial(ll, lr) == h61.partial(rl, rr)) {
                        sum = mod.plus(sum, ni);
                    }
                }

                io.cache.append(sum / 1000 % 10);
                io.cache.append(sum / 100 % 10);
                io.cache.append(sum / 10 % 10);
                io.cache.append(sum / 1 % 10);
                io.cache.append('\n');
            }
        }
    }
