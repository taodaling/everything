...
public class BZOJ1096 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            optimizer.add(0, 0, 0);
            for (int i = 1; i <= n; i++) {
                int j = optimizer.getBestChoice(di[i]);
                dp[i] = bi[i] + di[i] * si[i - 1]
                        - fi[i - 1] + dp[j] - di[i] * si[j] + fi[j];
                optimizer.add(dp[i] + fi[i], si[i], i);
            }

            int zeroUntil = n;
            while (zeroUntil > 0 && ai[zeroUntil] == 0) {
                zeroUntil++;
            }
            long min = Long.MAX_VALUE;
            for (int i = n; i >= zeroUntil; i--) {
