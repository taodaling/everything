...
public class BZOJ1113 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = n - 1; i >= 0; i--) {
                if (!next.containsKey(rects[i])) {
                    rights[i] = -1;
                } else {
                    rights[i] = next.get(rects[i]);
                }
                next.put(rects[i], i);
            }

            boolean[] visited = new boolean[n];
            St<Integer> st = new St(rects, n, new Comparator<Integer>() {
                @Override
            ...
            for (int i = 0; i < n; i++) {
                ...
                visited[i] = true;
                int trace = i;
                while (rights[trace] != -1) {
                    trace = rights[trace];
                    if (st.query(i, trace) < rects[i]) {
                        break;
                    }
                    visited[trace] = true;
                }
