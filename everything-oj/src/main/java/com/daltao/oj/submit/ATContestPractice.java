...
public class ATContestPractice {
    ...
    public static class Task implements Runnable {
        ...
        public void fordJohnson() {
            char[] order = new char[]{'A', 'B', 'C', 'D', 'E'};
            if (compare(order[0], order[1]) > 0) {
                Memory.swap(order, 0, 1);
            }
            if (compare(order[2], order[3]) > 0) {
                Memory.swap(order, 2, 3);
            }
            if (compare(order[1], order[3]) > 0) {
                Memory.swap(order, 0, 2);
                Memory.swap(order, 1, 3);
            }
            ...
            list.add(order[3]);
            list.add('+');
            binaryInsert(list, order[4]);

            boolean lastIsD = list.indexOf(order[3]) > list.indexOf(order[4]);
            list.remove((Character)order[3]);
            binaryInsert(list, order[2]);
            if (lastIsD) {
                list.add(order[3]);
            } else {
                list.add(list.indexOf(order[4]), order[3]);
            }

            answer(list);
        }

        public void binaryInsert(List<Character> list, char c) {
            int l = 1;
            int r = list.size() - 1;
            while (l < r) {
                int m = (l + r) >> 1;
                int compRes = compare(c, list.get(m));
                if (compRes < 0) {
                    r = m;
                } else {
                    l = m + 1;
        ...
        public int compare(char a, char b) {
            ...
            io.cache.append("? ").append(a).append(' ').append(b).append('\n');
            io.flush();
            return io.readChar() == '>' ? 1 : -1;
        }
    }
