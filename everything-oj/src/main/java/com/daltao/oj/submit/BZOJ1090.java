...
public class BZOJ1090 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            n = s.length();
            hash = new Hash(s.toCharArray(), 31);
            g = new int[n][n + 1];
            f = new int[n][n];
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
            ...
            io.cache.append(f(0, n - 1));
        }

        public int g(int i, int j) {
            if (g[i][j] == -1) {
                g[i][j] = 1;
                if (i + j + j <= n && hash.hash(i, i + j - 1) == hash.hash(i + j, i + j + j - 1)) {
                    g[i][j] += g(i + j, j);
                }
            }
            return g[i][j];
        ...
        public int f(int i, int j) {
            if (f[i][j] == -1) {
                ...
                for (int d = 1; d <= dist; d++) {
                    ...
                    f[i][j] = Math.min(f[i][j], f(i, i + d - 1) + 2 + numberOfDigits(blocks));
                }
            }
            return f[i][j];
        }
