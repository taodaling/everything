...
public class BZOJ1845 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int n = io.readInt();
            final GeometryUtils.LineConvexHull[] polygons = new GeometryUtils.LineConvexHull[n];
            List<GeometryUtils.Line2D> lines = new ArrayList();
            TreeSet<Double> distinctX = new TreeSet(new Comparator<Double>() {
                @Override
                public int compare(Double o1, Double o2) {
                    return GeometryUtils.compare(o1, o2);
                }
            });
            for (int i = 0; i < n; i++) {
                GeometryUtils.Point2D pt1 = new GeometryUtils.Point2D(io.readDouble(), io.readDouble());
                GeometryUtils.Point2D pt2 = new GeometryUtils.Point2D(io.readDouble(), io.readDouble());
                GeometryUtils.Point2D pt3 = new GeometryUtils.Point2D(io.readDouble(), io.readDouble());
                distinctX.add(pt1.x);
                distinctX.add(pt2.x);
                distinctX.add(pt3.x);
                polygons[i] = new GeometryUtils.GrahamScan(new GeometryUtils.PointPolygon(Arrays.asList(pt1, pt2, pt3)))
                        .convex.asLines();
                lines.addAll(polygons[i].data);
            }

            for (int i = 0, until = lines.size(); i < until; i++) {
                for (int j = 0; j < i; j++) {
                    GeometryUtils.Line2D a = lines.get(i);
                    GeometryUtils.Line2D b = lines.get(j);
                    GeometryUtils.Point2D pt = a.intersect(b);
                    if (pt == null) {
                        continue;
                    }
                    distinctX.add(pt.x);
            ...
            SimpsonRule.Function function = new SimpsonRule.Function() {
                ...
                public double y(double x) {
                    GeometryUtils.Line2D vectical = new GeometryUtils.Line2D(new GeometryUtils.Point2D(x, 0),
                            new GeometryUtils.Point2D(x, inf));
                    List<double[]> list = new ArrayList();
                    for (GeometryUtils.LineConvexHull p : polygons) {
                        double min = inf;
                        double max = 0;
                        for (int i = 0; i < 3; i++) {
                            GeometryUtils.Line2D line = p.data.get(i);
                            GeometryUtils.Point2D pt = line.intersect(vectical);
                            if (pt == null || !p.inside(pt, true)) {
                                continue;
                            }
            ...
            //function.y(1.5);
            double last = distinctX.first();
            double total = 0;
            SimpsonRule simpsonRule = new SimpsonRule(inf);
            for (double x : distinctX) {
                total += simpsonRule.integral(last, x, function);
                last = x;
            }

            total -= GeometryUtils.PREC;
            io.cache.append(String.format("%.2f", total));
        }
