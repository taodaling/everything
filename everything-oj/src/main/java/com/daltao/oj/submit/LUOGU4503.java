...
public class LUOGU4503 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            int l = io.readInt();
            int s = io.readInt();
            char[][] names = new char[n][l];
            Hash[] hashTemplates = new Hash[]{
                    new Hash(l, 31), new Hash(l, 71)
            };
            Hash[][] hashes = new Hash[n][2];
            for (int i = 0; i < n; i++) {
                io.readString(names[i], 0);
                for (int j = 0; j < 2; j++) {
                    hashes[i][j] = new Hash(hashTemplates[j]);
                    hashes[i][j].populate(names[i], l);
                }
            }

            Map<Long, Integer> cntMap = new LinkedHashMap<>(n);

            int same = 0;
            for (int i = 0; i < l; i++) {
                cntMap.clear();
                for (int j = 0; j < n; j++) {
                    Long h = concat(hashIgnoreIth(hashes[j][0], i),
                            hashIgnoreIth(hashes[j][1], i));
        ...
        public int hashIgnoreIth(Hash h, int i) {
            if (i > 0) {
                return Hash.MOD.subtract(h.hash[h.n - 1], Hash.MOD.subtract(h.hash[i], h.hash[i - 1]));
            } else {
                return Hash.MOD.subtract(h.hash[h.n - 1], h.hash[0]);
            }
        }
    }
