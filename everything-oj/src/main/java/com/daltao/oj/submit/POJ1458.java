...
public class POJ1458 {
    ...
    public static class Task implements Runnable {
        ...
        char[] m = new char[100000];
        int[] lowest = new int[300000];
        ...
        public void solve() {
            int N = io.readString(n, 0);
            int M = io.readString(m, 0);
            ...
            for (int d = 0; ; d++) {
                for (int t = -d; t <= d; t += 2) {
                    int traceIndex = t + h;
                    lowest[traceIndex] = Math.max(lowest[traceIndex - 1] + 1, lowest[traceIndex + 1]);
                    while (lowest[traceIndex] < N && lowest[traceIndex] - t < M && n[lowest[traceIndex]] == m[lowest[traceIndex] - t]) {
                        lowest[traceIndex]++;
                    }
                    if (lowest[traceIndex] == N && t == N - M) {
                        io.cache.append((N + M - d) / 2).append('\n');
                        return;
                    }
                }
            }
