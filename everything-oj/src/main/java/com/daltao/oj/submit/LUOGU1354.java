...
public class LUOGU1354 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            List<GeometryUtils.Segment2D> walls = new ArrayList<>();
            List<GeometryUtils.Point2D> pts = new ArrayList<>();
            pts.add(new GeometryUtils.Point2D(0, 5));
            pts.add(new GeometryUtils.Point2D(10, 5));

            for (int i = 0; i < n; i++) {
                double x = io.readDouble();
                double a1 = io.readDouble();
                double b1 = io.readDouble();
                double a2 = io.readDouble();
                double b2 = io.readDouble();

                GeometryUtils.Point2D p1 = new GeometryUtils.Point2D(x, -1);
                GeometryUtils.Point2D p2 = new GeometryUtils.Point2D(x, a1);
                GeometryUtils.Point2D p3 = new GeometryUtils.Point2D(x, b1);
                GeometryUtils.Point2D p4 = new GeometryUtils.Point2D(x, a2);
                GeometryUtils.Point2D p5 = new GeometryUtils.Point2D(x, b2);
                GeometryUtils.Point2D p6 = new GeometryUtils.Point2D(x, 11);


                walls.add(new GeometryUtils.Segment2D(p1, p2));
                walls.add(new GeometryUtils.Segment2D(p3, p4));
                walls.add(new GeometryUtils.Segment2D(p5, p6));
                pts.add(p2);
                pts.add(p3);
                pts.add(p4);
                pts.add(p5);
            ...
            for (int i = 0; i < m; i++) {
                GeometryUtils.Point2D from = pts.get(i);
                for (int j = i + 1; j < m; j++) {
                    GeometryUtils.Point2D to = pts.get(j);
                    if (GeometryUtils.near(from.x, to.x)) {
                        continue;
                    }
                    GeometryUtils.Segment2D seg = new GeometryUtils.Segment2D(from, to);
                    boolean blocked = false;
                    for (GeometryUtils.Segment2D wall : walls) {
                        blocked = blocked || block(wall, seg);
                    }
                    if (blocked) {
                        continue;
                    }
            ...
            TreeSet<Node> nodeSet = new TreeSet<>((a, b) -> a.dist == b.dist ? a.id - b.id : Double.compare(a.dist, b.dist));
            nodes[0].dist = 0;
            for (Node node : nodes) {
                nodeSet.add(node);
            }
            ...
            while (!nodeSet.isEmpty()) {
                Node head = nodeSet.pollFirst();
                if (head.dist == 1e50) {
                    continue;
                }
                for (Edge e : head.edges) {
                    Node other = e.other(head);
                    if (head.dist + e.len >= other.dist) {
                        continue;
                    }
                    nodeSet.remove(other);
                    other.dist = head.dist + e.len;
                    nodeSet.add(other);
                }
            }

            io.cache.append(String.format("%.2f", nodes[1].dist));
        }

        public boolean block(GeometryUtils.Segment2D wall, GeometryUtils.Segment2D vec) {
            GeometryUtils.Point2D intersect = wall.intersect(vec);
            return intersect != null && !intersect.near(vec.a) &&
                    !intersect.near(vec.b);
        }
    }
    ...
    public static class Edge {
        ...
        public String toString() {
            return String.format("(%s, %s, %.2f)", a.toString(), b.toString(), len);
        }
    }
