...
public class LOJ122V3 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            int n = io.readInt();
            int m = io.readInt();
            DynamicConnectionChecker g = new DynamicConnectionChecker(n, m);
            for (int i = 0; i < m; i++) {
                //debug.debug("i", i);
                int op = io.readInt();
                int x = (io.readInt() ^ last) - 1;
                int y = (io.readInt() ^ last) - 1;
                if (op == 0) {
                    g.addEdge(x, y);
                } else if (op == 1) {
                    g.deleteEdge(x, y);
                } else {
                    boolean connected = g.check(x, y);
                    io.cache.append(connected ? "Y" : "N").append('\n');
                    last = (connected ? x : y) + 1;
                }
            }
        }
    }
