    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            while (true) {
                ...
                for (int i = 0; i < n; i++) {
                    scores[i][0] = io.readInt();
                }
                for (int i = 0; i < n; i++) {
                    scores[i][1] = io.readInt();
                }
                ...
                while (r - l >= 1e-4) {
                    double mid = (r + l) / 2;
                    arrangeFirstKLargestScores(scores, 0, n, k, mid);
                    double sum = 0;
                    for (int i = k; i < n; i++) {
                        sum += calc(scores[i], mid);
                    }
                    if (sum >= 0) {
                        l = mid;
                    } else {
                ...
                io.cache.append((int) (l * 100 + 0.5)).append('\n');
            }
        }


        public static double calc(int[] score, double c) {
            return score[0] - score[1] * c;
        }

        public static int compare(int[][] scores, int i, int j, double c) {
            return Double.compare(calc(scores[i], c), calc(scores[j], c));
        }

        public static void arrangeFirstKLargestScores(int[][] scores, int from, int to, int k, double c) {
            if (to == from) {
                return;
            }
            Memory.swap(scores, from, Randomized.nextInt(from, to - 1));
            int l = from;
            int m = from + 1;
            int r = to;
            for (; m < r; ) {
                int diff = compare(scores, l, m, c);
                if (diff == 0) {
                    m++;
                } else if (diff > 0) {
                    Memory.swap(scores, m, l);
                    m++;
                    l++;
                } else {
                    Memory.swap(scores, m, r - 1);
                    r--;
                }
            }
            if (k <= l - from) {
                arrangeFirstKLargestScores(scores, from, l, k, c);
            } else if (k <= m - from) {
            } else {
                arrangeFirstKLargestScores(scores, m, to, k - (m - from), c);
            }
        }
    }
