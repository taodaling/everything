...
public class BZOJ3280 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve(int t) {
            ...
            m = io.readInt();
            k = io.readInt();
            MinCostMaxFlow zkw = new MinCostMaxFlow(idOfDst());
            long total = 0;
            for (int i = 0; i < n; i++) {
                int a = io.readInt();
                total += a;
                zkw.getChannel(idOfDay(i), idOfDst(), 0).modify(a, 0);
                zkw.getChannel(idOfSrc(), idOfDayRecover(i), 0).modify(a, 0);
                if (i + 1 < n) {
                    zkw.getChannel(idOfDay(i), idOfDay(i + 1), 0).modify(inf, 0);
                }
            }
            for (int i = 0; i < m; i++) {
                int l = io.readInt();
                int p = io.readInt();
                zkw.getChannel(idOfSrc(), idOfDay(0), p).modify(l, 0);
            }
            for (int i = 0; i < k; i++) {
                int d = io.readInt();
                int q = io.readInt();
                for (int j = 0; j < n; j++) {
                    if (j + 1 + d < n) {
                        zkw.getChannel(idOfDayRecover(j), idOfDay(j + 1 + d), q)
                                .modify(inf, 0);
                    }
                }
            }

            zkw.setSource(idOfSrc());
            zkw.setTarget(idOfDst());
            double[] res = zkw.send(inf);
            long flow = (long) (res[0] + 0.5);
            long fee = (long) (res[1] + 0.5);

            io.cache.append("Case ").append(t).append(": ");
            if (flow < total) {
                io.cache.append("impossible").append('\n');
            } else {
                io.cache.append(fee).append('\n');
            }
        }
    }
