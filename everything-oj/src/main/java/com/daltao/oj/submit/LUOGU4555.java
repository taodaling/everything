...
public class LUOGU4555 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            PalindromeAutomaton l2r = new PalindromeAutomaton(n);
            PalindromeAutomaton r2l = new PalindromeAutomaton(n);

            int[] tail = new int[n];
            int[] head = new int[n];
            for (int i = 0; i < n; i++) {
                l2r.build(s[i]);
                tail[i] = l2r.buildLast.len;
            }
            for (int i = n - 1; i >= 0; i--) {
                r2l.build(s[i]);
                head[i] = r2l.buildLast.len;
            }

            int max = 0;
            for (int i = 0; i < n - 1; i++) {
                max = Math.max(max, tail[i] + head[i + 1]);
            }
