...
public class POJ1755V3 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            double prec = 1e-12;
            for (int i = 1; i <= n; i++) {
                LinearProgramming.DualLinearProgramming lp = new LinearProgramming.DualLinearProgramming(n + 1, 3, prec);
                for (int j = 1; j <= n; j++) {
                    if (j == i) {
                        continue;
                    }
                    for (int k = 1; k <= 3; k++) {
                        lp.setConstraintCoefficient(j, k, 1.0 / speeds[j][k] - 1.0 / speeds[i][k]);
                    }
                    lp.setConstraintConstant(j, 2 * prec);
                }
                for (int j = 1; j <= 3; j++) {
                    lp.setConstraintCoefficient(n + 1, j, 1);
                }
                lp.setConstraintConstant(n + 1, 2 * prec);
                lp.solve();
                debug.debug("lp", lp);
                if (lp.isInfeasible()) {
                    io.cache.append("No\n");
