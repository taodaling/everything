...
public class CFECR47D {
    public static void main(String[] args) throws Exception {
        ...
        FastIO io = local ? new FastIO(new FileInputStream("E:\\DATABASE\\TESTCASE\\CFECR47D.in"), System.out, charset) : new FastIO(System.in, System.out, charset);
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            m -= n - 1;
            for (int i = 2; i <= n; i++) {
                io.cache.append(1).append(' ').append(i).append('\n');
            }

            for (int i = 2; i <= n && m > 0; i++) {
                for (int j = 2; j < i && m > 0; j++) {
                    if (Mathematics.gcd(i, j) == 1) {
                        io.cache.append(i).append(' ').append(j).append('\n');
                        m--;
                    }
                }
            }
