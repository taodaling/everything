...
public class CFCR493C {
    public static void main(String[] args) throws Exception {
        ...
        FastIO io = local ? new FastIO(new FileInputStream("E:\\DATABASE\\TESTCASE\\CFCR493C.in"), new FileOutputStream("E:\\DATABASE\\TESTCASE\\CFCR493C.out"), charset) : new FastIO(System.in, System.out, charset);
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            p[0][0] = 1;
            for (int i = 1; i <= n; i++) {
                p[i][0] = (int) ((long) p[i - 1][0] * i % MOD);
            }
            for (int i = 0; i <= n; i++) {
                p[i][1] = Mathematics.inverse(p[i][0], MOD);
            }

            int[] exp3 = new int[n + 1];
            exp3[0] = 1;
            for (int i = 1; i <= n; i++) {
                exp3[i] = (int) (3L * exp3[i - 1] % MOD);
            }

            for (int i = 1; i <= n; i++) {
                sum += powNegOne(i + 1) * comp(p, n, i) % MOD * exp3[i] % MOD * Mathematics.pow(3, (long) (n - i) * n, MOD) % MOD;
            }
            sum = sum * 2 % MOD;

            for (int i = 0; i < n; i++) {
                int exp3_i = MOD - exp3[i];
                sum += 3L * comp(p, n, i) % MOD * powNegOne(i + 1) % MOD * Mathematics.mod(Mathematics.pow(Mathematics.mod(1 + exp3_i, MOD), n, MOD) - Mathematics.pow(exp3_i, n, MOD), MOD) % MOD;
            }

            io.cache.append(sum % MOD);
        }

        public static long powNegOne(int n) {
            return (n & 1) == 1 ? (MOD - 1) : 1;
        }

        public static long comp(int[][] p, int n, int m) {
            return (long) p[n][0] * p[n - m][1] % MOD * p[m][1] % MOD;
        }
    }
    ...
    public static class Mathematics {
         ...
         * Get x^n(% mod)
         */
        public static int pow(int x, long n, int mod) {
            return pow(x, (int) (n % (mod - 1)), mod);
        }
