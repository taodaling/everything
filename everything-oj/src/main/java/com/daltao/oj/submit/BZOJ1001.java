...
public class BZOJ1001 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 1; i < n; i++) {
                for (int j = 1; j < m; j++) {
                    addEdge(idOf(i - 1, j - 1),
                            idOf(i, j), io.readInt());
                }
            }

            isap.setSource(idOf(0, 0));
            isap.setTarget(idOf(n - 1, m - 1));
            isap.bfs(new ArrayDeque());
            int total = isap.sendFlow(inf);
            debug.debug("isap", isap);
            io.cache.append(total);
        }

        void addEdge(int i, int j, int cap) {
            isap.buildChannel(i, j, cap, 0);
            isap.buildChannel(j, i, cap, 0);
        }
    }
    ...
    public static class ISAP {
        ...
        public static class InverseChannelWrapper implements Channel {
            ...
            @Override
            public String toString() {
                return String.format("\n%s--%s/%s-->%s", getSrc(), getFlow(), getCapacity(), getDst());
            }
        }
