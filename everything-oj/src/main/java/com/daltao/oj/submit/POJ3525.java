...
public class POJ3525 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve(int n) {
            ...
            GeometryUtils.PointPolygon pp = new GeometryUtils.PointPolygon(Arrays.asList(points));
            GeometryUtils.LinePolygon lp = pp.asLines();
            GeometryUtils.Point2D[] moveDirection = new GeometryUtils.Point2D[n];
            for (int i = 0; i < n; i++) {
                moveDirection[i] = lp.data.get(i).d.getApeak().normalize();
            }

            double l = 0;
            double r = 10000;
            GeometryUtils.Line2D[] fixedLines = new GeometryUtils.Line2D[n];
            while (r - l > 1e-6) {
                double m = (r + l) / 2;
                for (int i = 0; i < n; i++) {
                    fixedLines[i] = lp.data.get(i).moveAlong(moveDirection[i], m);
                }
                boolean exist = new GeometryUtils.HalfPlaneIntersection(new GeometryUtils.LinePolygon(Arrays.asList(fixedLines)), true, true).hasSolution;
                if (exist) {
                    l = m;
                } else {
                    r = m;
            ...
            io.cache.append(String.format("%.6f", l)).append('\n');
        }
    }
    ...
    public static class GeometryUtils {
        ...
        public static class Area {
            ...
            public double areaOfConvex(Polygon<Point2D> convexHull) {
                double area = 0;
                int n = convexHull.data.size();
                for (int i = 2; i < n; i++) {
                    area += areaOfTriangle(convexHull.data.get(0), convexHull.data.get(i),
                            convexHull.data.get(i - 1));
                }
                return area;
            }
        ...
        public static class GrahamScan {
            ...
            public GrahamScan(PointPolygon pointPolygon) {
                ...
                convex = new PointPolygon(new ArrayList(stack));
            }
        }
        ...
        public static class HalfPlaneIntersection {
            ...
            public HalfPlaneIntersection(Polygon<Line2D> linePolygon, boolean close, boolean isAnticlockwise) {
                ...
                convex = new LinePolygon(new ArrayList(deque));
            }
