...
public class BZOJ1036 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 1; i <= n; i++) {
                LCTNode.makeRoot(nodes[i]);
                nodes[i].change(io.readInt());
            }
            int q = io.readInt();
            char[] cmds = new char[100];
            for (int i = 1; i <= q; i++) {
                io.readString(cmds, 0);
                if (cmds[0] == 'C') {
                    LCTNode root = nodes[io.readInt()];
                    LCTNode.makeRoot(root);
                    root.change(io.readInt());
                } else {
                    LCTNode a = nodes[io.readInt()];
                    LCTNode b = nodes[io.readInt()];
                    ...
                    LCTNode.splay(b);
                    if (cmds[1] == 'M') {
                        io.cache.append(b.max).append('\n');
                    } else {
                        io.cache.append(b.sum).append('\n');
                    }
                }
            }
        }
    ...
    public static class LCTNode {
        ...
        static {
            ...
            NIL.father = NIL;
            NIL.treeFather = NIL;
            NIL.w = NIL.max = Integer.MIN_VALUE;
        }
