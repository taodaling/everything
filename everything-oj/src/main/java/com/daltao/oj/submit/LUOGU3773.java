...
public class LUOGU3773 {
    ...
    public static class Task implements Runnable {
        ...
        final Debug debug;
        int inf = (int) 1e8;
        private Modular mod = new Modular(1000000007);
        ...
        public void solve() {
            ...
            for (int i = 0; i < n; i++) {
                ...
                while (sg.hasNext()) {
                    ...
                    cnt = mod.plus(cnt, prefix[y]);
                }
                prefix[x] = mod.plus(prefix[x], cnt);
                prefix[x] = mod.plus(prefix[x], 1);
            }

            int sum = 0;
            for (int i = 1; i <= limit; i++) {
                sum = mod.plus(sum, prefix[i]);
            }
            sum = mod.subtract(sum, n);
            io.cache.append(sum);
        }
    }
