...
public class BZOJ1221 {
    ...
    public static class Task implements Runnable {
        ...
        int idOfWaitForUsing(int i) {
            return i + 1;
        }

        int idOfUsed(int i) {
            return n + idOfWaitForUsing(i);
        }

        int idOfWaitForClean(int i) {
            return idOfUsed(i) + n;
        }
        ...
        public void solve() {
            ...
            MinCostMaxFlow flow = new MinCostMaxFlow(idOfDst());
            flow.addChannel(idOfSrc(), idOfWaitForUsing(0), f).modify(inf, 0);
            for (int i = 0; i < n; i++) {
                int x = io.readInt();
                flow.addChannel(idOfWaitForUsing(i), idOfUsed(i), 0).modify(inf, 0);
                flow.addChannel(idOfUsed(i), idOfDst(), 0).modify(x, 0);
                flow.addChannel(idOfSrc(), idOfWaitForClean(i), 0).modify(x, 0);
                if (i + a + 1 < n) {
                    flow.addChannel(idOfWaitForClean(i), idOfWaitForUsing(i + a + 1), fa).modify(inf, 0);
                }
                if (i + b + 1 < n) {
                    flow.addChannel(idOfWaitForClean(i), idOfWaitForUsing(i + b + 1), fb).modify(inf, 0);
                }
                if (i > 0) {
                    flow.addChannel(idOfWaitForUsing(i - 1), idOfWaitForUsing(i), 0).modify(inf, 0);
                }
            }

            flow.setSource(idOfSrc());
            flow.setTarget(idOfDst());
            double[] flows = flow.send(inf);
            long fee = (long) (flows[1] + 0.5);
            io.cache.append(fee);
        }
    }
