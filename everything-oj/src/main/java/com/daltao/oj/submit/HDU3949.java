...
public class HDU3949 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 1; i <= t; i++) {
                io.cache.append("Case #").append(i).append(":").append(System.lineSeparator());
                linearBasis.clear();
                int n = io.readInt();
                for (int j = 0; j < n; j++) {
                    linearBasis.add(io.readLong());
                }
                long maxK = (1L << linearBasis.size()) - 1;
                int m = io.readInt();
                for (int j = 0; j < m; j++) {
                    long k = io.readLong();
                    //zero able
                    if (linearBasis.size() < n) {
                        k--;
                    }
                    if (k > maxK) {
                        io.cache.append(-1).append(System.lineSeparator());
                    } else {
                        io.cache.append(linearBasis.theKthSmallestNumber(k)).append(System.lineSeparator());
                    }
                }
            }
        }
