...
public class POJ1755 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int n = io.readInt();
            double[][] coes = new double[3][n];
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < 3; j++) {
                    coes[j][i] = 1.0d / io.readInt();
                }
            }

            for (int i = 0; i < n; i++) {
                List<GeometryUtils.Line2D> lineList = new ArrayList(n);
                lineList.add(new GeometryUtils.Line2D(new GeometryUtils.Point2D(0, 0),
                        new GeometryUtils.Point2D(1, 0)));
                lineList.add(new GeometryUtils.Line2D(new GeometryUtils.Point2D(0, 1),
                        new GeometryUtils.Point2D(0, 0)));
                for (int j = 0; j < n; j++) {
                    if (j == i) {
                        continue;
                    }
                    lineList.add(createLine(GeometryUtils.valueOf(coes[0][i] - coes[0][j]),
                            GeometryUtils.valueOf( coes[1][i] - coes[1][j]), GeometryUtils.valueOf(coes[2][i] - coes[2][j])));
                }

                GeometryUtils.HalfPlaneIntersection hpi = new GeometryUtils.HalfPlaneIntersection(new GeometryUtils.LinePolygon(lineList), false);
                if (hpi.hasSolution) {
                    io.cache.append("Yes\n");
                } else {
                    io.cache.append("No\n");
         ...
         * Find ax + by + c < 0
         */
        public GeometryUtils.Line2D createLine(double a, double b, double c) {
            GeometryUtils.Point2D pt1 = null;
            GeometryUtils.Point2D pt2 = null;
            if (a == 0 && b == 0) {
                if (c < 0) {
                    pt1 = new GeometryUtils.Point2D(0, 0);
                    pt2 = new GeometryUtils.Point2D(1, 0);
                    return new GeometryUtils.Line2D(pt1, pt2);
                } else {
                    pt1 = new GeometryUtils.Point2D(1, 0);
                    pt2 = new GeometryUtils.Point2D(0, 0);
                    return new GeometryUtils.Line2D(pt1, pt2);
                }
            } else if (a != 0) {
                pt1 = new GeometryUtils.Point2D(-c / a, 0);
                pt2 = new GeometryUtils.Point2D((-c - b) / a, 1);
                if (a < 0) {
                    return new GeometryUtils.Line2D(pt2, pt1);
                } else {
                    return new GeometryUtils.Line2D(pt1, pt2);
                }
            } else {
                pt1 = new GeometryUtils.Point2D(0, -c / b);
                pt2 = new GeometryUtils.Point2D(1, (-c - a) / b);
                if (b < 0) {
                    return new GeometryUtils.Line2D(pt1, pt2);
                } else {
                    return new GeometryUtils.Line2D(pt2, pt1);
