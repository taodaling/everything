...
public class LG4238 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int n = io.readInt();
            int proper = 32 - Integer.numberOfLeadingZeros(n - 1);
            int[] p = new int[1 << (proper + 1)];
            for (int i = 0; i < n; i++) {
                p[i] = io.readInt();
            }

            int[] inv = new int[1 << (proper + 1)];
            NumberTheoryTransform.inverse(new int[1 << (proper + 1)], p, inv, new int[1 << (proper + 1)], proper);
            for (int i = 0; i < n; i++) {
                io.cache.append(inv[i]).append(' ');
            }
