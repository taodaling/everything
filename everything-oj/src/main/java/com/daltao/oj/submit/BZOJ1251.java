...
public class BZOJ1251 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 0; i < n; i++) {
                ...
                splay = Splay.add(splay, node);
            }
            for (int i = 0; i < m; i++) {
                int k = io.readInt();
                int l = io.readInt();
                int r = io.readInt();
                splay = Splay.selectKthAsRoot(splay, r);
                Splay right = Splay.splitRight(splay);
                splay = Splay.selectKthAsRoot(splay, l);
                Splay left = Splay.splitLeft(splay);

                if (k == 1) {
                    splay.plus(io.readInt());
                } else if (k == 2) {
                    splay.reverse();
                } else {
                    io.cache.append(splay.max).append('\n');
                }
                splay = Splay.merge(left, splay);
    ...
    public static class Splay implements Cloneable {
        ...
        static {
            ...
            NIL.father = NIL;
            NIL.size = 0;
            NIL.max = NIL.key = Integer.MIN_VALUE;
        }
