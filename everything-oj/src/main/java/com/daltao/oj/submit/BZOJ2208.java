...
public class BZOJ2208 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (Node node : nodes) {
                for (Node next : node.next) {
                    node.set.nextSet.add(next.set);
                }
                root.nextSet.add(node.set);
            }
        ...
        public void dp(Node root) {
            if (root.visited) {
                return;
            }
            root.dp = root.size;
            root.visited = true;
            for (Node node : root.nextSet) {
                if (node == root) {
                    continue;
                }
                dp(node);
    ...
    public static class Node {
        ...
        Set<Node> nextSet = new LinkedHashSet(2);
        Node set;
        int dfn;
        int low;
        boolean instk;
