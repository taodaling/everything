...
public class AGC001E {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 0; i < n; i++) {
                grid[2000 - pair[i][0]][2000 - pair[i][1]]++;
            }

            for (int i = 0; i <= 4000; i++) {
                for (int j = 0; j <= 4000; j++) {
                    if (j > 0) {
                        grid[i][j] = mod.plus(grid[i][j], grid[i][j - 1]);
                    }
                    if (i > 0) {
                        grid[i][j] = mod.plus(grid[i][j], grid[i - 1][j]);
                    }
                }
            }
            ...
            int sum = 0;
            for (int i = 0; i < n; i++) {
                sum = mod.plus(sum, grid[2000 + pair[i][0]][2000 + pair[i][1]]);
                sum = mod.subtract(sum, comp.composite(2 * (pair[i][0] + pair[i][1]), pair[i][0] * 2));
            }

            io.cache.append(mod.mul(sum, pow.inverse(2)));
        }
    }
