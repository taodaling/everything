...
public class BZOJ1067 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int n = io.readInt();
            List<GeometryUtils.Point2D> pts = new ArrayList(n);
            for (int i = 0; i < n; i++) {
                GeometryUtils.Point2D pt = new GeometryUtils.Point2D(io.readDouble(), io.readDouble());
                pts.add(pt);
            }
            GeometryUtils.GrahamScan gs = new GeometryUtils.GrahamScan(new GeometryUtils.PointPolygon(pts));
            GeometryUtils.PointConvexHull pch = gs.convex;

            double ans = 0;
            for (int i = 0, until = pch.data.size(); i < until; i++) {
                ans = Math.max(ans, maxArea(pch, i));
            }

            io.cache.append(String.format("%.3f", ans));
        }

        public double maxArea(GeometryUtils.PointConvexHull pch, int startPoint) {
            GeometryUtils.Area area = new GeometryUtils.Area();

            //旋转卡壳
            int n = pch.data.size();

            GeometryUtils.Point2D[] ab = new GeometryUtils.Point2D[2];
            GeometryUtils.Point2D[] cd = new GeometryUtils.Point2D[]{pch.pointAtLoop(startPoint + 2), pch.pointAtLoop(startPoint + 3)};
            GeometryUtils.Point2D[] ef = new GeometryUtils.Point2D[]{pch.pointAtLoop(startPoint), pch.pointAtLoop(startPoint + 1)};

            double maxArea = 0;
            for (int i = startPoint + 1, j = startPoint + 3, k = startPoint + 2; k < startPoint + n - 1; k++) {
                ab[0] = pch.pointAtLoop(startPoint);
                ab[1] = pch.pointAtLoop(k);
                while (GeometryUtils.Point2D.cross(ab[0], ab[1], cd[0], cd[1]) >= 0 && GeometryUtils.Point2D.cross(ab[0], ab[1], cd[1], pch.pointAtLoop(j + 2)) >= 0) {
                    j++;
                    cd[0] = cd[1];
                    cd[1] = pch.pointAtLoop(j + 1);
                }
                while (GeometryUtils.Point2D.cross(ab[0], ab[1], ef[0], ef[1]) <= 0 && GeometryUtils.Point2D.cross(ab[0], ab[1], ef[1], pch.pointAtLoop(i + 2)) <= 0) {
                    i++;
                    ef[0] = ef[1];
                    ef[1] = pch.pointAtLoop(i + 1);
                }

                double localArea = Math.max(area.areaOfTriangle(ab[0], ab[1], cd[0]),
                        area.areaOfTriangle(ab[0], ab[1], cd[1]))
                        + Math.max(area.areaOfTriangle(ab[0], ab[1], ef[0]),
                        area.areaOfTriangle(ab[0], ab[1], ef[1]));
