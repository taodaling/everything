...
public class BZOJ1046 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            DiscreteMap map = new DiscreteMap(vals.clone(), 0, n);
            for (int i = 0; i < n; i++) {
                vals[i] = map.rankOf(vals[i]);
            }
            Segment root = new Segment(map.minRank(), map.maxRank());
            for (int i = n - 1; i >= 0; i--) {
                dp[i] = root.query(vals[i] + 1, map.maxRank(), map.minRank(), map.maxRank()) + 1;
                root.update(vals[i], vals[i], map.minRank(), map.maxRank(), dp[i]);
            }
            int m = io.readInt();
            for (int i = 0; i < m; i++) {
                int l = io.readInt();
                ...
                for (int j = 0; j < n && curLen > 0; j++) {
                    ...
                    curVal = vals[j];
                    curLen--;
                    io.cache.append(map.iThElement(curVal)).append(' ');
                }
                if (curLen > 0) {
    ...
    public static class Segment implements Cloneable {
        ...
        public void pushDown() {
            if (maxDirty != -1) {
                left.setMax(maxDirty);
                right.setMax(maxDirty);
                maxDirty = -1;
            }
        }
