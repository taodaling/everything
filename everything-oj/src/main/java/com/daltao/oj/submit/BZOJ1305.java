...
public class BZOJ1305 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    relations[i][j] = io.readChar() == 'Y';
                }
            }
            isap = new ISAP(idOfDst());
        ...
        int idOfInputBoy(int x) {
            return x + 1;
        }

        int idOfOutBoyLike(int x) {
            return idOfInputBoy(x) + n;
        }

        int idOfOutBoyDislike(int x) {
            return idOfOutBoyLike(x) + n;
        }

        int idOfInputGirlLike(int x) {
            return idOfOutBoyDislike(x) + n;
        }

        int idOfInputGirlDislike(int x) {
            return idOfInputGirlLike(x) + n;
        }

        int idOfOutGirl(int x) {
            return idOfInputGirlDislike(x) + n;
        }

        int idOfSrc() {
            return idOfOutGirl(n - 1) + 1;
        }
        ...
        public boolean test(int t) {
            for (int i = 0; i < n; i++) {
                isap.getChannel(idOfSrc(), idOfInputBoy(i)).reset(t, 0);
                isap.getChannel(idOfInputBoy(i), idOfOutBoyLike(i)).reset(inf, 0);
                isap.getChannel(idOfInputBoy(i), idOfOutBoyDislike(i)).reset(k, 0);
                isap.getChannel(idOfOutGirl(i), idOfDst()).reset(t, 0);
                isap.getChannel(idOfInputGirlLike(i), idOfOutGirl(i)).reset(inf, 0);
                isap.getChannel(idOfInputGirlDislike(i), idOfOutGirl(i)).reset(k, 0);
            }

            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    if (relations[i][j]) {
                        isap.getChannel(idOfOutBoyLike(i), idOfInputGirlLike(j)).reset(1, 0);
                    } else {
                        isap.getChannel(idOfOutBoyDislike(i), idOfInputGirlDislike(j)).reset(1, 0);
                    }
                }
            }
            ...
            return Math.abs(isap.sendFlow(inf) - t * n) < 0.5;
        }
    }
