...
public class CFCR497B {
    public static void main(String[] args) throws Exception {
        ...
        FastIO io = local ? new FastIO(new FileInputStream("E:\\DATABASE\\TESTCASE\\CFCR497B.in"), System.out, charset) : new FastIO(System.in, System.out, charset);
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int limit = (int) 1e5;
            int[] factorNums = new int[limit + 1];
            ...
            for (int i = 0; i < n; i++) {
                ...
                int ab = Mathematics.gcd(a, b);
                int bc = Mathematics.gcd(b, c);
                int ac = Mathematics.gcd(a, c);
                int abc = Mathematics.gcd(ab, bc);

                long value = (long) factorNums[a] * factorNums[b] * (long) factorNums[c]
                        - factorNums[ab] * factorNums[bc] * factorNums[ac]
                        - factorNums[ac] * factorNums[ab] * factorNums[bc]
                        + factorNums[abc] * factorNums[abc] * factorNums[abc];

                io.cache.append(value).append('\n');
            }
        }
    }
