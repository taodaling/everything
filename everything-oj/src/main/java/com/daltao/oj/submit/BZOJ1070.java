        ...
        public void solve() {
            ...
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < m; j++) {
                    int t = io.readInt();
                    for (int k = 0; k < n; k++) {
                        costFlow.addChannel(idOfCar(i), idOfPerson(j, k), (k + 1) * t).modify(1, 0);
                    }
                }
            }

            for (int i = 0; i < n; i++) {
                costFlow.addChannel(idOfSrc(), idOfCar(i), 0).modify(1, 0);
            }

            for (int i = 0; i < m; i++) {
                for (int j = 0; j < n; j++) {
                    costFlow.addChannel(idOfPerson(i, j), idOfDst(), 0).modify(1, 0);
                }
            }
            ...
            costFlow.send(inf);
            io.cache.append(String.format("%.2f", costFlow.fee / n));
        }
    }
