...
public class BZOJ1391 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            int src = n + m + 1;
            int dst = n + m + 2;
            ISAP isap = new ISAP(dst);
            isap.setSource(src);
            isap.setTarget(dst);
            for (int i = 1; i <= n; i++) {
                int profit = io.readInt();
                total += profit;
                int processNum = io.readInt();
                isap.getChannel(src, taskId(i)).modify(profit, 0);
                for (int j = 1; j <= processNum; j++) {
                    int m = io.readInt();
                    int rent = io.readInt();
                    isap.getChannel(taskId(i), machineId(m)).modify(rent, 0);
                }
            }
            for (int i = 1; i <= m; i++) {
                isap.getChannel(machineId(i), dst).modify(io.readInt(), 0);
            }

            int minCut = isap.sendFlow(inf);
            io.cache.append(total - minCut);
        }
    }
