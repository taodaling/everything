...
public class BZOJ1911 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            long c = io.readInt();
            int[] xi = new int[n + 1];
            long[] si = new long[n + 1];
            for (int i = 1; i <= n; i++) {
                xi[i] = io.readInt();
                si[i] = si[i - 1] + xi[i];
            }
            SlopeOptimizer optimizer = new SlopeOptimizer();
            optimizer.add(0, 0, 0);
            long[] dp = new long[n + 1];
            for (int i = 1; i <= n; i++) {
                int j = optimizer.getBestChoice(si[i]);
                dp[i] = a * si[i] * si[i] + b * si[i] + c -
                        (2 * a * si[i] * si[j] + b * si[j] - dp[j] - a * si[j] * si[j]);
                optimizer.add(b * si[i] - dp[i] - a * si[i] * si[i],
                        -2 * a * si[i], i);
            }
    ...
    public static class SlopeOptimizer {
