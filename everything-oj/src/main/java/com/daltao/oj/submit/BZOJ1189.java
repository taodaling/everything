...
public class BZOJ1189 {
    ...
    public static class Task implements Runnable {
        ...
        void spfa(Cell src) {
            ...
            while (!deque.isEmpty()) {
                Cell head = deque.removeFirst();
                for (int[] dir : dirs) {
                    if (head.x + dir[0] < 0 || head.x + dir[0] >= n ||
                            head.y + dir[1] < 0 || head.y + dir[1] >= m) {
                        continue;
                    }
                    Cell cell = map[head.x + dir[0]][head.y + dir[1]];
                    if (cell.c == 'X') {
                        continue;
                    }
                    if (cell.dist != -1) {
        ...
        public int idOfPeople(int i) {
            return i + 1;
        }

        public int idOfDoor(int i, int time) {
            return (time - 1) * doorCnt + peopleCnt + i + 1;
        }

        public int idOfSrc() {
            return peopleCnt + doorCnt * maxTime + 1;
        }
        ...
        public void solve() {
            ...
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < m; j++) {
                    ...
                    map[i][j].x = i;
                    map[i][j].y = j;
                    map[i][j].c = io.readChar();
                    if (map[i][j].c == '.') {
                        map[i][j].id = people.size();
                        people.add(map[i][j]);
                    } else if (map[i][j].c == 'D') {
                        map[i][j].id = doors.size();
                        doors.add(map[i][j]);
                    }
                }
            }

            for (Cell person : people) {
                spfa(person);
                ...
                for (int i = 0; i < n; i++) {
                    for (int j = 0; j < m; j++) {
                        ...
                        List<Cell> cells = person.doors.get(map[i][j].dist);
                        if (cells == null) {
                            cells = new ArrayList(1);
                            person.doors.put(map[i][j].dist, cells);
                        }
                        cells.add(map[i][j]);
                    }
                }

                if (person.doors.isEmpty()) {
        ...
        public boolean able(int time) {
            ...
            for (int i = 1; i <= time; i++) {
                for (Cell cell : doors) {
                    isap.getChannel(idOfDoor(cell.id, i), idOfDst()).modify(1, 0);
                    if (i > 1) {
                        isap.getChannel(idOfDoor(cell.id, i - 1), idOfDoor(cell.id, i)).modify(inf, 0);
                    }
                }
            }

            for (Cell person : people) {
                isap.getChannel(idOfSrc(), idOfPeople(person.id)).modify(1, 0);
                for (int i = 1; i <= time; i++) {
                    List<Cell> accessible = person.doors.get(i);
                    if (accessible == null) {
                        continue;
                    }
                    for (Cell door : accessible) {
                        isap.getChannel(idOfPeople(person.id), idOfDoor(door.id, i)).modify(1, 0);
                    }
                }
            }

            int flow = (int) (isap.sendFlow(inf) + 0.5);
            return flow == peopleCnt;
        }
    }
    ...
    public static class Cell {
        ...
        char c;
        int id;
        Map<Integer, List<Cell>> doors = new HashMap();
