...
public class CFCR497E {
    public static void main(String[] args) throws Exception {
        ...
        FastIO io = local ? new FastIO(new FileInputStream("E:\\DATABASE\\TESTCASE\\CFCR497E.in"), System.out, charset) : new FastIO(System.in, System.out, charset);
        ...
        public void solve() {
            ...
            for (int i = 0; i < t; i++) {
                ...
                if (j >= 0) {
                    ...
                    long adjust = sum + cur[j] - cap[j];
                    long carNeed = (adjust + k - 1) / k;
                    cnt += carNeed;
                    cur[j] -= carNeed * k - sum;

                    int x = j + 1;
                    while (cur[j] < 0 && x < n) {
                        long remove = Math.min(-cur[j], cur[x]);
                        cur[x] -= remove;
                        cur[j] += remove;
                        x++;
                    }

                    cur[j] = Math.max(0, cur[j]);
                }
