...
public class UOJ179 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            int n = io.readInt();
            int t = io.readInt();
            LinearProgramming lp = new LinearProgramming(n, m);
            for (int i = 1; i <= m; i++) {
                lp.setTargetCoefficient(i, io.readInt());
            }
            for (int i = 1; i <= n; i++) {
                for (int j = 1; j <= m; j++) {
                    lp.setConstraintCoefficient(i, j, io.readInt());
                }
                lp.setConstraintConstant(i, io.readInt());
            }

            debug.debug("lp", lp);
            lp.solve();
            if (lp.isInfeasible()) {
                io.cache.append("Infeasible");
            } else if (lp.isUnbound()) {
                io.cache.append("Unbounded");
            } else {
                io.cache.append(lp.bestSolution()).append("\n");
                if (t == 1) {
                    for (int j = 1; j <= m; j++) {
                        io.cache.append(lp.getAssignmentValueForVariable(j)).append(' ');
                    }
                }
            }
        }
