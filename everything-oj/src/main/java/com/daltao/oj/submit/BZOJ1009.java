...
public class BZOJ1009 {
    ...
    public static class ACAutomaton {
        ...
        static final int MAX_CHARACTER = '9';
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            ModMatrix transform = new ModMatrix(statusNum, statusNum);
            for (ACAutomaton.Node node : automaton.allNodes) {
                for (ACAutomaton.Node next : node.next) {
                    if (!next.word) {
                        transform.mat[next.id][node.id] += 1;
                    }
                }
            }

            ModMatrix vector = new ModMatrix(statusNum, 1);
            vector.mat[0][0] = 1;

            ModMatrix ans = ModMatrix.mul(ModMatrix.pow(transform, n, modular), vector, modular);

            int total = 0;
            for (int[] row : ans.mat) {
                total = modular.plus(total, row[0]);
            }
