        ...
        public void solve() {
            ...
            for (int i = 1; i <= n; i++) {
                ...
                if (t == 1) {
                    nodes[i].alloc = 1;
                    nodes[i].last = last;
                    nodes[i].since = nodes[i];
                    last = nodes[i];
                    nodes[i].c = io.readChar();
                } else {
                    nodes[i].since = nodes[io.readInt()].since;
                    nodes[i].offset = nodes[i].since.alloc;
                    nodes[i].c = io.readChar();
                }
            }
            ...
            char[] s = new char[n];
            for (int i = 2; i <= n; i++) {
                if (nodes[i].since == nodes[i].since) {
                    nodes[i].index = nodes[i].last.index + nodes[i].last.alloc;
                    nodes[i].l = nodes[i].index;
                } else {
                    nodes[i].index = nodes[i].since.index + nodes[i].offset;
                    nodes[i].l = nodes[i].since.l;
                }
                s[nodes[i].index] = nodes[i].c;
            }
            ...
            for (int i = 0; i < m; i++) {
                ...
                int cnt = 0;
                for (int j = which.l; j + len - 1 <= which.index; j++) {
