...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        NumberTheory.Composite composite = new NumberTheory.Composite(1000000, mod);
        ...
        public void solve() {
            ...
            int leftAlloc = M;
            int rightAlloc = N - M;
            int[] rightParts = new int[limit + 1];
            for (int i = 0; i <= limit; i++) {
                rightParts[i] = f(i, rightAlloc);
            }
            for (int i = 1; i <= limit; i++) {
                rightParts[i] = mod.plus(rightParts[i], rightParts[i - 1]);
            }
            int sum = 0;
            for (int i = 0; i < limit && limit - i - 1 >= 0; i++) {
                int plus = mod.mul(f(i, leftAlloc), rightParts[limit - i - 1]);
                sum = mod.plus(plus, sum);
            }
            sum = mod.mul(sum, composite.composite(N, M));
            sum = mod.subtract(power.pow(limit + 1, N), sum);
            io.cache.append(sum);
        }

        public int f(int i, int n) {
            return mod.subtract(power.pow(i + 1, n), power.pow(i, n));
        }
    }
