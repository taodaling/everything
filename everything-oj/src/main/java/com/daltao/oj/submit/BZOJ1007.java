...
public class BZOJ1007 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            Map<ConvexHullTrick.Line, Integer> inverseIndex = new IdentityHashMap(n);
            for (int i = 1; i <= n; i++) {
                double a = io.readDouble();
                double b = io.readDouble();
                inverseIndex.put(cht.insert(a, b), i);
            }

            boolean[] canSee = new boolean[n + 1];
            for (ConvexHullTrick.Line line : cht) {
                canSee[inverseIndex.get(line)] = true;
            }
            for (int i = 1; i <= n; i++) {
                if (canSee[i]) {
