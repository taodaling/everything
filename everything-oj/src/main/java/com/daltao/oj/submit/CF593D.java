...
public class CF593D {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 1; i < n; i++) {
                ...
                LCTNode edge = edges[i];
                long x = io.readLong();
                LCTNode.modVal(edge, x);
                LCTNode.join(a, edge);
                LCTNode.join(edge, b);
            }
            ...
            for (int i = 0; i < m; i++) {
                ...
                if (t == 1) {
                    ...
                    LCTNode.splay(a);
                    io.cache.append(y / a.prod).append('\n');
                } else {
                    LCTNode edge = edges[io.readInt()];
                    long c = io.readLong();
                    LCTNode.modVal(edge, c);
                }
            }
    ...
    public static class LCTNode {
        ...
        public static void modVal(LCTNode x, long val) {
            makeRoot(x);
            access(x);
            x.val = x.prod = val;
        }
        ...
        public void pushUp() {
            prod = mul(mul(left.prod, right.prod), val);
        }
    }
