...
public class POJ3384 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 0; i < n; i++) {
                points[n - 1 - i] = new GeometryUtils.Point2D(io.readDouble(), io.readDouble());
            }
            GeometryUtils.PointConvexHull pch = new GeometryUtils.PointConvexHull(Arrays.asList(points));
            GeometryUtils.LineConvexHull lch = pch.asLines();
            GeometryUtils.Line2D[] fixedLine = new GeometryUtils.Line2D[n];
            for (int i = 0; i < n; i++) {
                fixedLine[i] = lch.data.get(i).moveAlong(lch.data.get(i).d.getApeak().normalize(), r);
            }
            GeometryUtils.PointConvexHull afterShrink = new GeometryUtils.HalfPlaneIntersection(new GeometryUtils.LinePolygon(Arrays.asList(fixedLine)), true, true)
                    .convex.asPoints();
            GeometryUtils.Segment2D farthestPointPair = afterShrink.theFarthestPointPairBruteForce();
            if (farthestPointPair.a.x > farthestPointPair.b.x) {
                farthestPointPair = new GeometryUtils.Segment2D(farthestPointPair.b, farthestPointPair.a);
            }
            io.cache.append(String.format("%.6f %.6f %.6f %.6f", farthestPointPair.a.x,
                    farthestPointPair.a.y, farthestPointPair.b.x, farthestPointPair.b.y)).append('\n');
        }
    }
    ...
    public static class GeometryUtils {
        ...
        public static class LineConvexHull extends LinePolygon {
            ...
            public PointConvexHull asPoints() {
                ...
                if (deque.size() >= 2) {
                    Point2D tail = deque.get(deque.size() - 1);
                    Point2D head = deque.get(0);
                    if (valueOf(head.x - tail.x) == 0 && valueOf(head.y - tail.y) == 0) {
        ...
        public static class PointConvexHull extends PointPolygon {
            ...
            public Segment2D theFarthestPointPairBruteForce() {
                ...
                Point2D x = null;
                Point2D y = null;
                double farthestDist2 = -1;
                for (int i = 0; i < n; i++) {
                    for (int j = 0; j < n; j++) {
                        Point2D a = data.get(i);
                        Point2D b = data.get(j);
                        double dist2 = a.distance2Between(b);
            ...
            public Segment2D theFarthestPointPair() {
                ...
                for (int i = 0, j = 1; i < n; i++) {
                    ...
                    while ((j + 1) % n != i && Point2D.cross(ab[0], ab[1], cd[0], cd[1]) <= 0 && Point2D.cross(ab[0], ab[1], cd[1], pointAtLoop(j + 2)) <= 0) {
                        j++;
                        cd[0] = cd[1];
