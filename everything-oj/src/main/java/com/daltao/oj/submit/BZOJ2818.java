...
public class BZOJ2818 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            long total = 0;
            MathUtils.Euler euler = new MathUtils.Euler(n);

            long[] prefixSumOfEuler = new long[n / 2 + 1];
            prefixSumOfEuler[0] = euler.euler[0];
            for (int i = 1; i < prefixSumOfEuler.length; i++) {
                prefixSumOfEuler[i] = prefixSumOfEuler[i - 1] + euler.euler[i];
            }
            for (int i = 0; i < euler.primeLength; i++) {
                int p = euler.primes[i];
                total += 2 * prefixSumOfEuler[n / p] - 1;
            }
