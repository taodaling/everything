...
public class TIMUS1132 {
    ...
    public static class Task implements Runnable {
        ...
        NumberTheory.QuadraticResidue qs = new NumberTheory.QuadraticResidue(new NumberTheory.Modular(0));
        ...
        public void solve() {
            ...
            int x = qs.square(a);
            if (x == -1) {
                io.cache.append("No root\n");
                return;
            }
            int y = qs.modular.plus(p, -x);
            io.cache.append(Math.min(x, y)).append(' ')
                    .append(Math.max(x, y)).append('\n');
        }
    }
