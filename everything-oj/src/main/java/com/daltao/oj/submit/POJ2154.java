...
public class POJ2154 {
    ...
    public static class Task implements Runnable {
        ...
        public void initPrimes() {
            boolean[] isComp = new boolean[sqrt + 1];
            for (int i = 2; i <= sqrt; i++) {
                if (!isComp[i]) {
                    primes[primeCnt++] = i;
                }
                for (int j = 0; j < primeCnt && i * primes[j] <= sqrt; j++) {
                    int pi = primes[j] * i;
                    isComp[pi] = true;
                    if (i % primes[j] == 0) {
                        break;
        ...
        public void solve() {
            if (debug.allowDebug) {
                debug.assertTrue(phi(1) == 1);
                debug.assertTrue(phi(2) == 1);
                debug.assertTrue(phi(5) == 4);
                debug.assertTrue(phi(4) == 2);
            }

            int testcase = io.readInt();
            while (testcase-- > 0) {
                int n = io.readInt();
                mod = io.readInt();
                ...
                for (int i = 1; i * i <= n; i++) {
                    ...
                    ans = mod(ans + phi(n / i) * Mathematics.pow(n, i - 1, mod));
                    if (n / i != i) {
                        ans = mod(ans + phi(i) * Mathematics.pow(n, n / i - 1, mod));
                    }
                }

                //flip
           /*     if (n % 2 == 0) {
                    ans = mod(ans + flipByEdge(n) * mod(n / 2));
                    ans = mod(ans + flipByVertex(n) * mod(n / 2));
                } else {
                    ans = mod(ans + flipByVertex(n) * mod(n));
                }
*/
        ...
        public int flipByEdge(int n) {
            return Mathematics.pow(n, n / 2, mod);
        }

        public int flipByVertex(int n) {
            return Mathematics.pow(n, n / 2 + 1, mod);
        }
    }
