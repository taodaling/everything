...
public class AGC033E {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            io.cache.append(dp(n / 2, 1, (dist + 1) / 2));
        }
        ...
        public int dp2(int n, int t) {
            ...
            for (int i = 2; i < n; i++) {
                ...
                dp[i] = peek[i - 2];
                if (i - 2 * t >= 0) {
                    dp[i] = modular.plus(dp[i], -peek[i - 2 * t]);
                }
                odd[i] = odd[i - 1];
                even[i] = even[i - 1];
                peek[i] = modular.plus(dp[i], peek[i]);
            }
            for (int i = 1; i <= t && i * 2 <= n; i++) {
                int count = modular.mul(dp[n - i * 2], i * 2);
                dp[n] = modular.plus(dp[n], count);
            }
            return dp[n];
        }
