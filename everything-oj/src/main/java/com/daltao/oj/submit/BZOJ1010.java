...
public class BZOJ1010 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int n = io.readInt();
            int l = io.readInt();
            SlopeOptimize so = new SlopeOptimize();
            ...
            s1[0] = prefix[0] + 0 - l;
            s2[0] = prefix[0] + 0 + 1;
            so.add(dp[0] + s2[0] * s2[0], 2 * s2[0], 0);
            for (int i = 1; i <= n; i++) {
                prefix[i] = prefix[i - 1] + io.readInt();
                s1[i] = prefix[i] + i - l;
                s2[i] = prefix[i] + i + 1;
                int j = so.getBestChoice(s1[i]);
                dp[i] = s1[i] * s1[i] + dp[j] + s2[j] * s2[j] - 2 * s1[i] * s2[j];
                so.add(dp[i] + s2[i] * s2[i], 2 * s2[i], i);
            }
            io.cache.append(dp[n]);
        }
    }

    public static class SlopeOptimize {
