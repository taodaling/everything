...
public class LUOGU1341 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int n = io.readInt();
            EulerTrace eulerTrace = new EulerTrace(128);
            for (int i = 0; i < n; i++) {
                char a = io.readChar();
                char b = io.readChar();
                eulerTrace.addUndirectedEdge(a, b);
            }
            for (EulerTrace.Node node : eulerTrace.nodes) {
                node.edges.sort((a, b) -> a.other(node).id - b.other(node).id);
            }
            if (!eulerTrace.findTrace()) {
                io.cache.append("No Solution");
                return;
            }
            for (EulerTrace.Node node : eulerTrace.getEulerTrace()) {
                io.cache.append((char) node.id);
            }
            io.cache.reverse();
        }
    }
     ...
     */
    public static class EulerTrace {
