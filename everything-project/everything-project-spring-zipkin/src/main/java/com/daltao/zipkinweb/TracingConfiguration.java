...
public class TracingConfiguration implements WebMvcConfigurer {
     ...
     */
    @Bean
    Sender sender(@Value("${spring.application.name}") String serviceName) {
        return OkHttpSender.create("http://virtualbox.com:9411/api/v2/spans?serviceName=" + serviceName);
    }
     ...
     */
    @Bean
    Tracing tracing(@Value("${spring.application.name}") String serviceName, AsyncReporter<Span> spanReporter) {
